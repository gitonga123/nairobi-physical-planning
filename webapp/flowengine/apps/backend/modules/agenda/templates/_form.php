		    ...
		    <div class="col-md-12">
				<div class="panel panel-dark">
					...
					<div class="panel-body panel-body-nopadding">
						...
						<form action="<?php echo url_for('/backend.php/agenda/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
							...
							<tfoot>
							  <tr>
								<td colspan="2">
								  ...
								  &nbsp;<a href="<?php echo url_for('/backend.php/agenda/index') ?>" class="btn btn-warning">Back to list</a>
								  <?php if (!$form->getObject()->isNew()): ?>
...
$(function(){
	$('#agenda_columns_form_id').change(function(){
		...
		$.ajax({
			url: '<?php echo url_for('/backend.php/usercategories/updatememeberfieldsagenda')?>',
			data: {form:$(this).val()},
			type: "POST",
			dataType: "json",
		}).done(function(reps){
...
$('#agenda_columns_entry_column').change(function(e){
	$.each($(this).val(),function(i,x){
		if($('#'+x).length == 0){
			$.ajax({
				url: '<?php echo url_for('/backend.php/agenda/agendaapparray') ?>',
				type: "GET",
				dataType: 'json',
			}).done(function(resp){
				if($('#'+x).length == 0){
...
if(positions.length > 0){
	$.ajax({
		url: '<?php echo url_for('/backend.php/agenda/agendaarrays') ?>',
		type: "POST",
		dataType: 'json',
		data: {form: $('#agenda_columns_form_id').val()}
	}).done(function(resp){
