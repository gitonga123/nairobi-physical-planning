...
<div class="pageheader">
	<h2><i class="fa fa-th-list"></i> <?php echo __("Tasks"); ?>
		<span><?php echo $application->getApplicationId(); ?></span></h2>
	...
	<?php else: ?>
		<div class="panel panel-default">
			...
			if ($application->getApproved() != 0 && $current_stage->getStageType() != 3) //Only show if settings access and not a invoice stage
			{
								...
								if ($sf_user->mfHasCredential('accessmenu' . $stagegroup->getId())) {
									...
									foreach ($stages as $stage) {
										...
										if ($sf_user->mfHasCredential('accesssubmenu' . $stage->getId())) {
											...
											?>
											<li><a
													onClick="if(confirm('Are you sure you want to move it?')){ window.location = '/plan/applications/moveapp/id/<?php echo $application->getId(); ?>/stage/<?php echo $stage->getId(); ?>'; }"><?php echo $stage->getTitle(); ?></a>
											</li>
											<?php
										}
									}
									...
									/*if($stagecontent != "")
				{
					...
					<?php if ($sf_user->mfHasCredential('can_run_cyclic') && Functions::client_can_add_businesses()): ?>
						<a href="<?php echo url_for('/plan/applications/cyclicondemand/id/' . $application->getId()) ?>"
							class="btn btn-info">Cyclic Billing</a>
						<a href="<?php echo url_for('/plan/applications/cyclicondemandfuture/id/' . $application->getId()) ?>"
							class="btn btn-info">Cyclic Billing (+1 Year)</a>
						<a href="<?php echo url_for('/plan/applications/movetoworkflow/id/' . $application->getId()) ?>"
							class="btn btn-warning" onClick="confirm('Are you sure?')">Move Workflows</a>
					<?php endif; ?>
				</div>
			<?php } ?>
				...
				if ($application->getAssessmentInprogress() == 0 && $q->count() == 0) {
					...
					<div class="panel-heading text-right">
						<a class="btn btn-primary"
							href="/plan/tasks/pick/id/<?php echo $application->getId(); ?>"><?php echo __('Start Task') ?></a>
					</div>
					<?php
				} else {
					$task = $q->fetchOne();
					...
					if ($task) {
						...
						<div class="panel-heading text-right">
							<a class="btn btn-primary"
								href="/plan/tasks/view/id/<?php echo $task->getId(); ?>"><?php echo __('View Task') ?></a>
						</div>
						<?php
					}
				}
			...
			if ($sf_user->mfHasCredential("backend_share_application") && $otbhelper->isSharedStage($application->getApproved())):
				?>
				<div class="panel-heading text-right">
					<a title="<?php echo __('Share Application'); ?>" class="btn btn-warning"
						onClick="window.location='/plan/share/share/id/<?php echo $application->getId(); ?>';"><span
							class="glyphicon glyphicon-share"></span><?php echo __('Share Application') ?></a>
				</div>
			...
			<div class="panel-body">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h5 class="bug-key-title"><?php echo $application->getApplicationId(); ?> -
							<?php echo $application->getStatusName(); ?></h5>
						...
						<div class="btn-group pull-right" style="margin-top: -38px;">
							<?php if ($application->getStage()->getAllowEdit() && $sf_user->mfHasCredential("accesssubmenu" . $application->getApproved())) { ?>
								<a href="/plan/applications/edit/id/<?php echo $application->getId(); ?>"
									class="btn btn-primary"><i class="fa fa-edit"></i>
									<?php echo __("Edit Application"); ?></a>
								<?php
								$q = Doctrine_Query::create()
								...
								foreach ($links as $link) {
									...
									if ($sharedform) {
										...
										?>
										<a title="<?php echo __('Edit'); ?> <?php echo $shared_form->getFormName(); ?>"
											class="btn btn-primary"
											onClick="window.location='/plan/applications/editentries?form_id=<?php echo $link->getFormId() ?>&id=<?php echo $link->getEntryId(); ?>&form_entry_id=<?php echo $application->getId() ?>';"><span
												class="glyphicon glyphicon-edit"></span> <?php echo __('Edit Application'); ?></a>
										<?php
							...
							<div class="panel-heading">
								<ul class="nav nav-tabs">
									<li <?php if ($current_tab == "application") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=application"><span
												class="fa fa-bars"></span> <?php echo __('Application Details'); ?></a></li>
									<li <?php if ($current_tab == "application_link") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=application_link"><span
												class="fa fa-bars"></span>
											<?php echo __('Application Additional Details'); ?></a></li>
									<li <?php if ($current_tab == "revisions") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=revisions"><span
												class="fa fa-bars"></span> <?php echo __('Previous Revisions'); ?></a></li>
									<li <?php if ($current_tab == "reviews") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=reviews"><span
												class="fa fa-eye"></span> <?php echo __('Review History'); ?></a></li>
									<li <?php if ($current_tab == "messages") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=messages"><span
												class="fa fa-comments"></span> <?php echo __('Messages'); ?></a></li>
									<li <?php if ($current_tab == "memo") {
										echo "class='active'";
									} ?>><a href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=memo"><span
												class="fa fa-comments-o"></span> <?php echo __('Memo  '); ?><span
												class="badge"><?php echo $internal_memo; ?></span></a></li>
									<?php if ($sf_user->mfHasCredential("access_application_history")): ?>
										<li <?php if ($current_tab == "history")
											echo "class='active'"; ?>>
											<a
												href="/plan/applications/view?id=<?php echo $application->getId(); ?>&current_tab=history">
												<span class="fa fa-bars"></span> <?php echo __('Application History'); ?>
											</a>
										</li>
									<?php endif; ?>
								</ul>
							...
							<div class="panel-body">
								...
								<div class="panel panel-default">
									<div class="panel-heading panel-heading-noradius">
										<h4 class="panel-title">
											<a data-toggle="collapse" class="collapsed" data-parent="#accordion"
												href="#commentsConditions"><?php echo __("Conditions of Approval"); ?></a>
										</h4>
									</div>
