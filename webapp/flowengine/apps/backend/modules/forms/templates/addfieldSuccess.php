<?php
	...
	switch ($element_type) {
			...
			//OTB patch
			case 'plinth_area' 	: $element_title = 'Plinth Area';break;
			case 'total_plinth_area' 	: $element_title = 'Total Plinth Area';break;
			case 'dropdown_dynamic' 	: $element_title = 'Dropdown dynamic';break;
	}
	...
	if(($action == 'click_new') || ($action == 'drag_new')){
		...
		$element_properties['text_default_type'] = 'static';
		$element_properties['text_default_length'] = 10;
		$element_properties['text_default_random_type'] = 'letter';
		$element_properties['text_default_prefix'] = '';
		$element_properties['text_default_case'] = 'u';
		...
		$element_properties['section_display_in_email'] = 1;
		$element_properties['choice_max_entry'] = 0;
		//OTB patch - Used to idenfity element as footprint
		$element_properties['footprint'] 	= 0;
		$element_properties['file_ifc'] 	= 0;
		$element_properties['plot_no'] 	= 0;
		$element_properties['ownertype'] 	= 0;
		...
		if($element_type == 'checkbox'){
			$element_properties['choice_limit_rule'] 	= 'atleast';
			$element_properties['choice_limit_qty']		= 1;
			$element_properties['choice_limit_range_min'] 	= 1;
			$element_properties['choice_limit_range_max']	= 1;
		}
		...
		if(in_array($element_type,array('select','radio','checkbox'))){
			...
			//if this is dropdown, make sure to turn off the 'choice has other' flag
			//since dropdown field doesn't support that flag
	...
	if(in_array($element_type,array('radio','checkbox','select','matrix'))){
		if(($action == 'click_new') || ($action == 'drag_new')){ //if this is creating new field
				...
				for($i=1;$i<=3;$i++){
					$query = "INSERT INTO 
										`".MF_TABLE_PREFIX."element_options` 
											(`form_id`,`element_id`,`option_id`,`position`,`option_text`,`option_is_default`,`live`) 
								   VALUES ( ? , ? , ? , ? , ? ,'0','2');"; 
			...
			if(in_array($element_type,array('radio','checkbox','select'))){
				$query = "INSERT INTO 
									 ".MF_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option_text`,`option_is_default`,`live`)
						  	   SELECT 
						  	   		 `form_id`, '{$element_id}' as new_element_id,`option_id`,`position`,`option_text`,`option_is_default`,'2' as `live` 
						  	   	 FROM 
						  	   	 	 ".MF_TABLE_PREFIX."element_options 
						  	   	WHERE 
				...
				foreach($matrix_element_id_pair as $source_element_id=>$dest_element_id){
					$query = "INSERT INTO 
										 ".MF_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option_text`,`option_is_default`,`live`)
							  	   SELECT 
							  	   		 `form_id`, '{$dest_element_id}' as new_element_id,`option_id`,`position`,`option_text`,`option_is_default`,'2' as `live` 
							  	   	 FROM 
							  	   	 	 ".MF_TABLE_PREFIX."element_options 
							  	   	WHERE 
							  	   		 form_id = ? AND element_id = ? AND (live=1 OR live=2);"; 
