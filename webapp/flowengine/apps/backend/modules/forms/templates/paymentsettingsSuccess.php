<?php
	...
	if(!empty($row)){
		...
		$payment_properties->enable_merchant = $row['payment_enable_merchant'] == -1 || $row['payment_enable_merchant'] == 0?0:1;
		...
		$payment_properties->onsubmission   = $row['payment_onsubmission'];
		$payment_properties->merchant_identifier   = $row['payment_merchant_identifier'];
	...
	$currency_symbol=$payment_properties->currency;
	...
	error_log('--------currency_symbol------'.$currency_symbol);
	...
	$query = "select element_id from ".MF_TABLE_PREFIX."element_prices where form_id = ? group by element_id order by aep_id asc";
	$params = array($form_id);
	...
	while($row = mf_do_fetch_result($sth)){
		...
		if($row['element_type'] != 'money'){
			...
			while($sub_row = mf_do_fetch_result($sub_sth)){
				...
				$price_field_options_array[$element_id][$i]['option'] = htmlspecialchars(strip_tags($sub_row['option_text']));
				$price_field_options_lookup[$element_id][$sub_row['option_id']] = htmlspecialchars(strip_tags($sub_row['option_text']));
				$i++;
			}
					...
					<ul id="ps_main_list">
						<li>
							<div id="ps_box_merchant_settings" class="ps_box_main gradient_blue">
								...
								<div class="ps_box_content">
									<span>	
										<input id="ps_enable_merchant" class="checkbox" value="" type="checkbox" style="margin-left: 0px" <?php if($payment_properties->enable_merchant){ echo 'checked="checked"'; } ?>>
										<label class="choice" for="ps_enable_merchant">Enable Merchant</label> 
										<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="Disabling this option will turn off the payment functionality of your form."/>
									</span>
									
									<label class="description" for="ps_select_merchant" style="margin-top: 10px">
										Select a Merchant 
										<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="A merchant will process transactions on your form and authorize the payments."/>
									</label>
									<select class="select" id="ps_select_merchant" style="width: 60%" autocomplete="off">
										<option <?php if($payment_properties->merchant_type == 'pesaflow_standard'){ echo 'selected="selected"'; } ?> value="pesaflow_standard">Pesaflow Standard</option>
										<option <?php if($payment_properties->merchant_type == 'pesaflow_cart'){ echo 'selected="selected"'; } ?> value="pesaflow_cart">Pesaflow Cart</option>
										<option <?php if($payment_properties->merchant_type == 'braintree'){ echo 'selected="selected"'; } ?> value="braintree">Braintree (For Test Payments)</option>
										...
										?>
										<option <?php if($payment_properties->merchant_type == $merchant['name']){ echo 'selected="selected"'; } ?> value="<?php echo $merchant['name'] ?>"><?php echo $merchant['name'] ?></option>
										<?php
											endforeach;
										?>
									</select>
									<div id="ps_paypal_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'paypal_standard'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="ps_paypal_email">PayPal Email Address <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="This is the email address associated with your PayPal account."/></label>
										...
										<label class="description" for="ps_paypal_language">Language <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="Select the language to be displayed on PayPal pages."/></label>
										...
										<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top; margin-top: 12px" title="If enabled, all transactions will go through PayPal test server (Sandbox). You can use this to test payments without using actual money. You'll need to login to https://developer.paypal.com and create your sandbox account there."/>				
									</div>
									...
									<div id="ps_braintree_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'braintree'){ echo 'style="display: none"'; } ?>>
										<div id="ps_braintree_live_keys" <?php if(!empty($payment_properties->braintree_enable_test_mode)){ echo 'style="display: none;"'; } ?>>
											<label class="description" for="ps_braintree_live_merchant_id">Merchant ID <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are API keys provided by Braintree. You can see all your keys in your Braintree control panel. Login to your Braintree account and go to 'Your Username' -> 'My User' -> 'API Keys'. Copy all your live keys here."/></label>
										...
										<div id="ps_braintree_test_keys">
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="You need to signup for Braintree Sandbox account to use this. If enabled, all credit card transactions won't go through the actual credit card network. You can use some test cards numbers (provided by Braintree) to simulate a successful transaction."/>
											<div id="ps_braintree_test_keys_div" <?php if(empty($payment_properties->braintree_enable_test_mode)){ echo 'style="display: none;"'; } ?>>
													<label class="description" for="ps_braintree_test_merchant_id">Sandbox Merchant ID <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are API keys provided by Braintree. You can see all your keys in your Braintree control panel. Login to your Braintree account and go to 'Your Username' -> 'My User' -> 'API Keys'. Copy all your sandbox keys here."/></label>
									...
									<div id="ps_pesaflow_standard_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'pesaflow_standard'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="ps_pesaflow_api_id">API ID <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are API keys provided by Braintree. You can see all your keys in your Braintree control panel. Login to your Braintree account and go to 'Your Username' -> 'My User' -> 'API Keys'. Copy all your live keys here."/></label>
										<input id="ps_pesaflow_api_id" name="ps_pesaflow_api_id" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_checkout_url); ?>" type="text">

										<label class="description" for="ps_pesaflow_key">Pesaflow Key <span class="required">*</span></label>
										<input id="ps_pesaflow_key" name="ps_pesaflow_key" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_merchant_username); ?>" type="password">

										<label class="description" for="ps_pesaflow_secret">Pesaflow Secret <span class="required">*</span></label>
										<input id="ps_pesaflow_secret" name="ps_pesaflow_secret" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_merchant_password); ?>" type="password">

										<label class="description" for="ps_pesaflow_service_id">Service ID <span class="required">*</span></label>
										<input id="ps_pesaflow_service_id" name="ps_pesaflow_service_id" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_service_id); ?>" type="text">													
									</div>
									<div id="ps_pesaflow_cart_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'pesaflow_cart'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="ps_pesaflow_api_id">API ID <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are API keys provided by Braintree. You can see all your keys in your Braintree control panel. Login to your Braintree account and go to 'Your Username' -> 'My User' -> 'API Keys'. Copy all your live keys here."/></label>
										<input id="ps_pesaflow_api_id" name="ps_pesaflow_api_id" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_checkout_url); ?>" type="text">

										<label class="description" for="ps_pesaflow_key">Pesaflow Key <span class="required">*</span></label>
										<input id="ps_pesaflow_key" name="ps_pesaflow_key" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_merchant_username); ?>" type="text">

										<label class="description" for="ps_pesaflow_secret">Pesaflow Secret <span class="required">*</span></label>
										<input id="ps_pesaflow_secret" name="ps_pesaflow_secret" class="element text large" value="<?php echo htmlspecialchars($payment_properties->cellulant_merchant_password); ?>" type="text">
									...
									<div id="ps_paypal_rest_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'paypal_rest'){ echo 'style="display: none"'; } ?>>
										<div id="ps_paypal_rest_live_keys" <?php if(!empty($payment_properties->paypal_rest_enable_test_mode)){ echo 'style="display: none;"'; } ?>>
											<label class="description" for="ps_paypal_rest_live_clientid">Client ID <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are keys provided by PayPal. Login to https://developer.paypal.com and go to Applications -> My apps -> Create App. Follow the instruction and create an app with the name 'machform'. You'll get all your keys there."/></label>
										...
										<div id="ps_paypal_rest_test_keys">
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="If enabled, all credit card transactions won't go through the actual credit card network. You can use some test cards numbers (provided by PayPal) to simulate a successful transaction."/>
									...
									<div id="ps_stripe_options" class="merchant_options" <?php if($payment_properties->merchant_type != 'stripe'){ echo 'style="display: none"'; } ?>>
										<div id="ps_stripe_live_keys" <?php if(!empty($payment_properties->stripe_enable_test_mode)){ echo 'style="display: none;"'; } ?>>
											<label class="description" for="ps_stripe_live_secret_key">Live Secret Key <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are keys provided by Stripe. You can see all your keys in your Stripe dashboard. Login to your Stripe account and go to 'Your Account' -> 'Account Settings' -> 'API Keys'. Copy all your Live keys here."/></label>
										...
										<div id="ps_stripe_test_keys">
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="If enabled, all credit card transactions won't go through the actual credit card network. You can use some test cards numbers (provided by Stripe) to simulate a successful transaction."/>
											<div id="ps_stripe_test_keys_div" <?php if(empty($payment_properties->stripe_enable_test_mode)){ echo 'style="display: none;"'; } ?>>
													<label class="description" for="ps_stripe_test_secret_key" style="margin-top: 10px">Test Secret Key <img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="These are keys provided by Stripe. You can see all your keys in your Stripe dashboard. Login to your Stripe account and go to 'Your Account' -> 'Account Settings' -> 'API Keys'. Copy all your Test keys here."/></label>
									...
									<div id="ps_stripe_info" class="merchant_options" <?php if($payment_properties->merchant_type != 'stripe'){ echo 'style="display: none"'; } ?>>
										Stripe is an easy way to accept credit card payments. <a class="blue_dotted" href="https://stripe.com/" target="_blank">Learn More</a>
									</div>
						...
						<li <?php if($payment_properties->enable_merchant === 0){ echo 'style="display: none;"'; } ?>>
							<div id="ps_box_payment_options" class="ps_box_main gradient_red">
								...
								<div class="ps_box_content">
									<div class="ps_onsubmission_div">	
										<input id="ps_onsubmission" class="checkbox" value="" type="checkbox" style="margin-left: 0px" <?php if($payment_properties->onsubmission){ echo 'checked="checked"'; } ?>>
										<label class="choice" for="ps_onsubmission">Payment on submission</label> 
										<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="Enable this for payment to be on submission"/>
									</div>
									<div class="ps_merchant_identifier_div">	
										<label class="choice" for="ps_merchant_identifier">Payment merchant identifier</label>
										<img class="helpmsg" src="/form_builder/images/icons/68_blue.png" style="vertical-align: top" title="Merchant alphanumeric identifier that will not change during the lifecylce of applications"/>
										<input id="ps_merchant_identifier" class="element text large" type="text"  value="<?php echo $payment_properties->merchant_identifier ?>">
									</div>
									<div id="ps_currency_paypal_div" <?php if($payment_properties->merchant_type != 'paypal_standard'){ echo 'style="display: none"'; } ?>>
										...
										Currency
										<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Select the currency you would like to use to accept the payment from your clients."/>
										</label>
									...
									<div id="ps_currency_pesaflow_standard_div" <?php if($payment_properties->merchant_type != 'pesaflow_standard'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="ps_currency_pesaflow_standard" style="margin-top: 2px">
										Currency
										<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Select the currency you would like to use to accept the payment from your clients."/>
										</label>
										<select class="select large" id="ps_currency_pesaflow_standard" name="ps_currency_pesaflow_standard" autocomplete="off">
											<option value="KES" <?php if($payment_properties->currency == 'KES'){ echo 'selected="selected"'; } ?>>KES - Kenya Shilling</option>
											<option value="USD" <?php if($payment_properties->currency == 'USD'){ echo 'selected="selected"'; } ?>>&#36; - U.S. Dollar</option>
										</select>
									</div>

									<div id="ps_currency_pesaflow_cart_div" <?php if($payment_properties->merchant_type != 'pesaflow_cart'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="ps_currency_pesaflow_cart" style="margin-top: 2px">
										Currency
										<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Select the currency you would like to use to accept the payment from your clients."/>
										</label>
										<select class="select large" id="ps_currency_pesaflow_cart" name="ps_currency_pesaflow_cart" autocomplete="off">
											<option value="KES" <?php if($payment_properties->currency == 'KES'){ echo 'selected="selected"'; } ?>>KES - Kenya Shilling</option>
											<option value="USD" <?php if($payment_properties->currency == 'USD'){ echo 'selected="selected"'; } ?>>&#36; - U.S. Dollar</option>
										</select>
									</div>
									...
									<div id="merchant_currency" <?php if($payment_properties->merchant_type == 'check' || $payment_properties->merchant_type == 'pesaflow_standard' || $payment_properties->merchant_type == 'pesaflow_cart' || $payment_properties->merchant_type == 'braintree'){ echo 'style="display: none"'; } ?>>
										<label class="description" for="currency_check" style="margin-top: 2px">
										Currency
										<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Select the currency you would like to use to accept the payment from your clients."/>
										</label>
										<select class="select large" id="currency_check" name="currency_check" autocomplete="off">
										</select>
									</div>
									...
									<div id="ps_optional_settings">
										...
										<input id="ps_show_total_amount" <?php if($payment_properties->show_total){ echo 'checked="checked"'; } ?> class="checkbox" value="" type="checkbox" style="margin-left: 0px">
										<label class="choice" for="ps_show_total_amount">Show Total Amount</label>
										<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Shows the total amount of the payment to the client as they are filling out the form. You can also select the location of the total amount placement within the form."/>
										<div id="ps_show_total_location_div" <?php if(!$payment_properties->show_total){ echo 'style="display: none"'; } ?>>
												Display at 
										...
										<div class="paypal_option stripe_option authorizenet_option paypal_rest_option braintree_option pesaflow_standard_option pesaflow_cart_option" <?php if(!in_array($payment_properties->merchant_type,array('stripe','paypal_standard','authorizenet','paypal_rest','braintree', 'pesaflow_standard', 'pesaflow_cart'))){ echo 'style="display: none"'; } ?>>
											...
											<label class="choice" for="ps_enable_tax">Add Convenience Fee / Sales Tax</label>
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Upon checkout, sales tax will automatically be added to the order total. You can define the tax rate here."/>
											<div id="ps_tax_rate_div" <?php if(empty($payment_properties->enable_tax)){ echo 'style="display: none"'; } ?>>
													Convenience Fee / Tax Rate (%):  
										...
										<div class="paypal_option paypal_rest_option stripe_option authorizenet_option" <?php if(!in_array($payment_properties->merchant_type,array('stripe','paypal_standard','authorizenet','paypal_rest'))){ echo 'style="display: none"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="If enabled, your clients will be charged automatically for every period of time."/>
											<div id="ps_recurring_div" <?php if(empty($payment_properties->enable_recurring)){ echo 'style="display: none"'; } ?>>
												...
												<select id="ps_recurring_cycle_unit" <?php if(!in_array($payment_properties->merchant_type,array('paypal_standard','authorizenet','paypal_rest'))){ echo 'style="display: none"'; }; ?>>
										...
										<div class="stripe_option paypal_rest_option" id="ps_setupfee_div_container" <?php if(empty($payment_properties->enable_recurring) || !in_array($payment_properties->merchant_type, array('stripe','paypal_rest'))){ echo 'style="display: none"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="In addition of the recurring payments, your clients will be charged for a one-time setup fee as well."/>
										...
										<div class="paypal_option stripe_option authorizenet_option paypal_rest_option braintree_option" id="ps_discount_div_container">
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Allows your client to enter coupon code and receive discount."/>
											<div id="ps_discount_div" <?php if(empty($payment_properties->enable_discount)){ echo 'style="display: none"'; } ?>>
												<ul>
													...
													<li>
														<label class="description" for="ps_discount_code" style="margin-top: 0px">Coupon Code:  <img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Coupon codes are case insensitive. Use commas to separate multiple coupon codes."/></label>
													...
													<li>
														...
														<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="The coupon can only be used this many times in total. Enter '0' for unlimited usage."/>
													</li>
													<li id="ps_li_discount_expiry">
														...
														<span id="discount_expiry_cal">
																...
																<div style="display: none"><img id="discount_expiry_pick_img" alt="Pick date." src="/form_builder/images/icons/calendar.png" class="trigger" style="margin-top: 3px; cursor: pointer" /></div>
														</span>
													</li>
												</ul>
											</div>
										...
										<div style="clear: both;margin-top: 10px"></div>
										<div class="stripe_option" id="ps_stripe_receipt_div_container">
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="If enabled, your clients will receive email receipt directly from Stripe upon successful payments.<br/><br/>NOTE: Stripe doesn't send email receipts in Test Mode."/>
											<div id="ps_stripe_enable_receipt_div" <?php if(empty($payment_properties->stripe_enable_receipt)){ echo 'style="display: none"'; } ?>>
												<ul>
													<li>
														<label class="description" style="margin-top: 5px" for="ps_stripe_receipt_element_id">Select Email Receipt Field:  <img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Stripe receipt will be sent to the email address entered into this field. If your form doesn't have it, you need to add an email field first."/></label>
													...
													<li>
														<label class="description" for="ps_stripe_charge_desc" style="margin-top: 0px">Charge Description:<span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="Enter a descriptive name for the charge being included within the email receipt."/></label>
										...
										<span class="ps_options_span paypal_option stripe_option authorizenet_option paypal_rest_option braintree_option" <?php if(in_array($payment_properties->merchant_type,array('paypal_standard','stripe','authorizenet','paypal_rest','braintree'))){ echo 'style="display: block"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="By default, notification emails are being sent when payment is made successfully. If you disable this option (not recommended), notification emails are being sent immediately upon form submission, regardless of payment status."/>
										</span>
										...
										<span id="ask_billing_span" class="ps_options_span stripe_option authorizenet_option paypal_rest_option braintree_option" <?php if(in_array($payment_properties->merchant_type,array('stripe','authorizenet','paypal_rest','braintree'))){ echo 'style="display: block"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="If enabled, the payment page will prompt your clients to enter their billing address."/>
										</span>
										...
										<span id="ask_shipping_span" class="ps_options_span stripe_option authorizenet_option paypal_rest_option braintree_option" <?php if(in_array($payment_properties->merchant_type,array('stripe','authorizenet','paypal_rest','braintree'))){ echo 'style="display: block"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="If enabled, the payment page will prompt your clients to enter their shipping address."/>
										</span>
										...
										<span id="save_cc_data_span" class="ps_options_span authorizenet_option" <?php if(in_array($payment_properties->merchant_type,array('authorizenet'))){ echo 'style="display: block"'; } ?>>
											...
											<img class="helpmsg" src="/form_builder/images/icons/68_red.png" style="vertical-align: top" title="If enabled, all your customers data (including credit card numbers) will be stored into your account on Authorized.net server. You need to enable CIM on your Authorize.net account to use this feature."/>
										</span>
									...
									<div id="ps_box_price_fields">
										...
										<div>
											<span id="ps_price_name_container" <?php if($payment_properties->merchant_type == 'stripe' && !empty($payment_properties->stripe_enable_receipt)){ echo 'style="display: none;"'; } ?>>
												<label class="description" for="ps_price_name" style="margin-top: 15px">Price Name <span class="required">*</span> <img class="helpmsg" src="/form_builder/images/icons/68_green.png" style="vertical-align: top" title="Select the fee code"/></label>
												<select name="ps_price_name" id="ps_price_name" class="element select large">
													<?php 
													$q=Doctrine_Query::create()
														->from('FeeCode c')
														->groupBy('c.service_id');
													$codes=$q->execute();
													
													foreach($codes as $code):
													?>
													<option value="<?php echo $code->getServiceId() ?>" <?php echo $payment_properties->price_name == $code->getServiceId()? "selected":"" ?> data-amount="<?php echo $code->getAmount() ?>" data-fixed="<?php echo $code->getFixed() ?>"><?php echo $code->getServiceName()?></option>
													<?php endforeach; ?>
												</select>
											</span>
										</div>
...
<script type="text/javascript" src="/form_builder/js/payment_settings1.1.js"></script>
EOT;
