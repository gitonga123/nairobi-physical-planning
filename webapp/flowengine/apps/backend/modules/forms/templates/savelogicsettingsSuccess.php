<?php
    ...
    $workflow_rule_properties = mf_sanitize($_POST['workflow_rule_properties']);
    $workflow_rule_conditions = mf_sanitize($_POST['workflow_rule_conditions']);
    ...
    $logic_workflow_enable = (int) $logic_statuses['logic_workflow_enable'];
	...
	/** Workflow Logic **/
    //save workflow_rule_properties into ap_workflow_logic_elements table
    $query = "delete from ".MF_TABLE_PREFIX."workflow_logic_elements where form_id=?";
    $params = array($form_id);
    mf_do_query($query,$params,$dbh);

    if(!empty($workflow_rule_properties)){
        $query = "insert into `".MF_TABLE_PREFIX."workflow_logic_elements`(form_id,element_id,rule_show_hide,rule_all_any) values(?,?,?,?)";
        foreach ($workflow_rule_properties as $data) {
            $params = array($form_id,$data['element_id'],strtolower($data['rule_show_hide']),strtolower($data['rule_all_any']));
            mf_do_query($query,$params,$dbh);
        }
    }

    //save workflow_rule_conditions into ap_workflow_logic_conditions table
    $query = "delete from ".MF_TABLE_PREFIX."workflow_logic_conditions where form_id=?";
    $params = array($form_id);
    mf_do_query($query,$params,$dbh);

    if(!empty($workflow_rule_conditions)){
        $query = "insert into `".MF_TABLE_PREFIX."workflow_logic_conditions`(form_id,target_element_id,element_name,rule_condition,rule_keyword) values(?,?,?,?,?)";
        foreach ($workflow_rule_conditions as $data) {
            $target_element_id = (int) $data['target_element_id'];
            $element_name	   = strtolower(trim($data['element_name']));
            $rule_condition    = strtolower(trim($data['condition']));
            $rule_keyword	   = trim($data['keyword']);
	...
	if(!empty($email_rule_properties) && !empty($email_rule_conditions)){
		...
		foreach ($email_rule_properties as $data) {
			...
			$custom_bcc		= trim($data['custom_bcc']);
	...
	$form_input['logic_workflow_enable']    = $logic_workflow_enable;
	...
	if(empty($workflow_rule_properties)){
		$form_input['logic_workflow_enable'] = 0; //always disable logics when target pages are empty
	}
