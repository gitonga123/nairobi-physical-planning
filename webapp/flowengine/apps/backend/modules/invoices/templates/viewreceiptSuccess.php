...
$param['checkbox_image'] = '/machform/images/icons/59_blue_16.png';
...
<div class="pageheader">
       <h2><i class="fa fa-edit"></i><?php echo __('My Receipts'); ?></h2>
       <div class="breadcrumb-wrapper">
            <ol class="breadcrumb">
               ...
               <li class="active"><?php
                            ...
                            $sql = "SELECT * FROM ext_translations WHERE field_id = '".$form_id."' AND field_name = 'form_name' AND table_class = 'ap_forms' AND locale = '".$_SESSION['locale']."'";
							$q = Doctrine_Query::create()
							   ->from("ExtTranslations a")
							   ->where("a.locale = ? AND a.field_id = ? AND a.field_name = ? AND a.table_class = ?", array($_SESSION['locale'],$form_id, 'form_name', 'ap_forms'));
							$translation = $q->fetchOne();
			                //$rows = mysql_query($sql, $dbconn);
			                if($translation)
			                {
				                $form_name = $translation->getTrlContent();
			                }
			                else
			                {
...
?>  
  	<tr>
  		...
  		<td><?php echo nl2br($data['value']); ?></td>
  	</tr>
<?php } ?>  	
  	
		<tr>
  	    <td><?php echo __('Date Uploaded'); ?> </td>
