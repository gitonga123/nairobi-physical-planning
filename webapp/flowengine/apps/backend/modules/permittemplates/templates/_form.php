...
<form  action="/plan/permittemplates/<?php echo ($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : ''); ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>   autocomplete="off" data-ajax="false" class="form-bordered">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <h3 class="panel-title"><?php echo ($form->getObject()->isNew()?__('New Permit Template'):__('Edit Permit Template'));?></h3>
    ...
    <div class="panel-body padding-0">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-12">
          ...
          <?php echo $form['parttype'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Page Type'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['page_type']->renderError() ?>
          <?php echo $form['page_type'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Page Orientation'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['page_orientation']->renderError() ?>
          <?php echo $form['page_orientation'] ?>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Expiration Trigger'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['expiry_trigger']->renderError() ?>
          <?php echo $form['expiry_trigger'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Allow issuance of previous year'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['allow_issue_last_paid']->renderError() ?>
          <?php echo $form['allow_issue_last_paid'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Check conditions before issue'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['check_conditions']->renderError() ?>
          <?php echo $form['check_conditions'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Allows Signing'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['allows_signing']->renderError() ?>
          <?php echo $form['allows_signing'] ?>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Remote URL (Update a remote database with the data from this permit)'); ?></label><br>
        <div class="col-sm-12">
      ...
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Remote Post Data (Actual data to be posted remotely)'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['remote_field']->renderError() ?>
          <?php echo $form['remote_field'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Remote Username (Username if the remote url requires authentication)'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['remote_username']->renderError() ?>
          <?php echo $form['remote_username'] ?>
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-4 control-label"><?php echo __('Remote Password (Password if the remote url requires authentication)'); ?></label><br>
        <div class="col-sm-12">
          <?php echo $form['remote_password']->renderError() ?>
          <input type="password" class="form-control" name="permits[remote_password]" value="<?php if(!$form->getObject()->isNew()){ echo $form->getObject()->getRemotePassword(); } ?>" id="permits_remote_password">
        </div>
      </div>
      <div class="form-group">
        <div class="col-sm-12">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          if($stage)
          {
			  //error_log("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS");
            ...
            echo "<tr><td><strong>".$apform->getFormName()." details</strong></td><td><strong>Tag</strong></td></tr>";
             ...
             foreach($elements as $element)
             {
               ...
               if($childs == 0)
               {
                  if($element->getElementType() == "select")
                  {
                      echo "<tr><td>".$element->getElementTitle()."</td><td>{fm_c_".$apform->getFormId()."_element_".$element->getElementId()."}</td></tr>";
                  }
                  else
                  {
                     echo "<tr><td>".$element->getElementTitle()."</td><td>{fm_c_".$apform->getFormId()."_element_".$element->getElementId()."}</td></tr>";
               ...
               else
               {
                   for($x = 0; $x < ($childs + 1); $x++)
                   {
                     echo "<tr><td>".$element->getElementTitle()."</td><td>{fm_c_".$apform->getFormId()."_element_".$element->getElementId()."_".($x+1)."}</td></tr>";
                   }
               }
             }
          }
          ...
          <table class="table dt-on-steroids mb0">
            ...
            <tbody>
              ...
              <tr><td><?php echo __('Balance Due'); ?></td><td>{inv_balance}</td></tr>
...
jQuery(document).ready(function(){
  ...
  jQuery('#permits_qr_content').ckeditor();
