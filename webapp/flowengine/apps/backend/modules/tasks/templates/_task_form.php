...
if (mf_is_form_submitted()) { //if form submitted
    ...
    error_log('------FORM SUBMITTED SUBMITTED RESULTS-------');
    error_log(var_export($submit_result, true));
    ...
    if (!isset($input_array['password'])) { //if normal form submitted
        ...
        if ($submit_result['status'] === true) {
            if (!empty($submit_result['form_resume_url'])) { //the user saving a form, display success page with the resume URL
                ...
                $_SESSION['review_id'] = $submit_result['review_id'];
                // header("Location: /plan/forms/confirm?id={$input_array['form_id']}{$page_num_params}");
                header("Location: http{$ssl_suffix}://" . "rms.nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . '/forms/confirm?id=' . $input_array['form_id'] . $page_num_params);
                exit;
            } else {
                if (!empty($submit_result['next_page_number'])) { //redirect to the next page number
                    ...
                    header("Location: http{$ssl_suffix}://" . "rms.nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . "?id={$input_array['form_id']}&mf_page={$submit_result['next_page_number']}");
                    exit;
                    ...
                    if (mf_is_payment_has_value($dbh, $input_array['form_id'], $submit_result['entry_id'])) {
                        ...
                        if (empty($submit_result['form_redirect'])) {
                            header("Location: http{$ssl_suffix}://" . "rms.nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . "?id={$input_array['form_id']}&done=1");
                            exit;
                        } else {
                            echo "<script type=\"text/javascript\">top.location.replace('{$submit_result['form_redirect']}')</script>";
                            exit;
    ...
    if (!empty($_GET['done']) && (!empty($_SESSION['mf_form_completed'][$form_id]) || !empty($_SESSION['mf_form_resume_url'][$form_id]))) {
        ...
        $task->setEndDate(date("Y-m-d H:m:s"));
