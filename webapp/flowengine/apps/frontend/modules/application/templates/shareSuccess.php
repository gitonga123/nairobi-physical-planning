             ...
             <h4><?php echo __('Share This Application With Other People') ?> </h4>
              ...
              <div class="panel-body-nopadding pt10">
				...
				<div class="alert alert-warning">
					...
					<p>If this is the user you wanted to share with, you can proceed to move the application for the user to access their required form(s). <a href="<?php echo url_for('/index.php/application/sharemove?architect='.$shared_user[0]['user_id'].'&id='.$application->getId()) ?>" class="btn btn-primary">Move Application</a></p>
				</div>
				<?php endif; ?>
...
foreach($pager->getResults() as $architect)
{
  ...
  if($user)
  {
	  //if($user->getIsActive() == "1" && $user->getIsSuperAdmin() == "1")
	  {
		...
		?>
		<td align="center">
           <a title='<?php echo __('Share Application')?>' href='<?php echo public_path('index.php/application/share/id/'.$application->getId().'/architect/'.$architect->getUserId()); ?>'>
