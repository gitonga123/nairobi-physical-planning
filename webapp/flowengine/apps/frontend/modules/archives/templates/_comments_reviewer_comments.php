<?php
/**
 * _comments_reviewer_comments template.
 *
 * Show full comments submitted by a reviewer
 *
 * @package    backend
...
//If applications has been circulated to one or more reviewers, continue
if($application->getCirculationId())
{
    ...
    if ($nConnection)
    {
        if (mysql_select_db(sfConfig::get('app_mysql_db'),$nConnection))
        {
            ...
            $strQuery = "SELECT * FROM cf_formtemplate WHERE nid = 7";
            ...
            //-----------------------------------------------	      
            $arrSlots = array();
            $strQuery = "SELECT * FROM cf_formslot WHERE ntemplateid = 7  ORDER BY nslotnumber ASC";
            $nResult = mysql_query($strQuery, $nConnection);
            if ($nResult)
            {
                if (mysql_num_rows($nResult) > 0)
            ...
            //-----------------------------------------------	
            $arrValues = array();
            $strQuery = "SELECT * FROM cf_fieldvalue WHERE nformid=".$application->getCirculationId()." AND nuserid = ".$task->getOwnerUserId()." AND nrevisionid = ".$task->getId();
            $nResult = mysql_query($strQuery, $nConnection);
            if ($nResult)
            {
                if (mysql_num_rows($nResult) > 0)
			...
			{
				$strQuery = "SELECT * FROM cf_fieldvalue WHERE nformid=".$application->getCirculationId()." AND nuserid = ".$task->getOwnerUserId();
				$nResult = mysql_query($strQuery, $nConnection);
				if ($nResult)
				{
					if (mysql_num_rows($nResult) > 0)
        ...
        if (mysql_select_db(sfConfig::get('app_mysql_db'),$nConnection))
        {
            foreach ($arrSlots as $arrSlot)
            {
                $q = Doctrine_Query::create()
					 ...
					 ->andWhere('a.nuserid = ?', $task->getOwnerUserId());
                $fielddata = $q->execute();
                            ...
                            if (mysql_num_rows($nResult) > 0)
                            {
                                ...
                                while($arrRow = mysql_fetch_array($nResult))
                                {
...

                                    {
                                        ...
                                        $strEcho = $objMyCirculation->getRadioGroup2($nInputfieldID,$application->getCirculationId(),$arrSlot['nid'], $strValue, $bIsEnabled, $keyId, $nRunningCounter, $task);
