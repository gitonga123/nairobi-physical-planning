			...
			if($submit_result['status'] === true){
				if(empty($submit_result['review_id'])){
					if(empty($submit_result['form_redirect'])){
						...
						header("Location: http{$ssl_suffix}://".$_SERVER['HTTP_HOST']."/index.php/frusers/index/done/1");
						exit;
					}else{
					...
					header("Location: /index.php/index/confirm?id={$input_array['form_id']}");
					exit;
				}
			}else{
...
<ul class="breadcrumb">
					...
					<li><a href="/index.php/frusers/index">Members</a></li>
