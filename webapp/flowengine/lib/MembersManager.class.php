...
class MembersManager
{
	...
	public function sendMemberVerificationMail($reg_no, $usercategory, $memberships)
	{
		...
		foreach ($validation_array as $k => $v) {
			...
			error_log("Verification Link ---->");
			error_log($validation_link);
	...
	public function MembershipIsValidated($user_id)
	{
		...
		if ($loggedInSfUserProfile && $loggedInSfUserProfile->getRegisteras()) {
			$q = Doctrine_Query::create()
		...
		if ($loggedInMfUserProfile && $UserCategory && $UserCategory->getMemberNoElementId()) {

			if (!is_null($loggedInMfUserProfile->getEntryId())) {
		...
		if ($UserCategory && $UserCategory->getId() == 6) {
			$member_no = 6;
		}
	...
	public function checkIfUserAccountIsActivated($form_id = null, $entry_id = null, $user_id = null)
	{
		...
		if (!$membership_database) {
			return false;// to enforce check revert back to false//uasin gishu fix
		}
