...
class mailnotifications
{
	...
	public function sendemail($from, $to, $subject, $body)
	{
		...
		try {
			...
			//$mail->SMTPDebug = 4;
			error_log("Start Sending ---> Email! ---> initial 1");
			error_log("Sending to ---->" . $to);
			error_log("SMTP ENABLE---->" . $apsettings->getSmtpEnable());
			error_log(strlen($apsettings->getSmtpHost()) && $apsettings->getSmtpEnable());
			error_log("Send");
			...
			$mail->setFrom($organisation_email, $organisation_name, 0);
			...
			//error_log("Checking if email sent ----><>" . $mail->send());
			//notification history
			...
			$ntf_arch->save();
			error_log("Message sent! ---> initial 1");
