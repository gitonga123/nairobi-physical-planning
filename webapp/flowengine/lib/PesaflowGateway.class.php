...
class PesaflowGateway {
    ...
    public function checkout_standard($application_id, $backend = false)
    {
        $invoice = null;
        ...
        {
            $payment_amount = $payment_amount + $detail->getAmount();
        }
...
EOT;
    ...
    public function ipn($request_details)
    {
      try
      {
        ...
        $invoice = $request_details["invoice"];
        $transaction_id = $request_details["transaction_id"];
		...
		$invoice_found=$this->invoice_manager->get_invoice_by_id($invoice);
		if($invoice_found){
        ...
        if($invoice_found->getPaid() <> 2)
        {
          $q = Doctrine_Query::create()
             ->from("ApFormPayments a")
             ->where("a.form_id = ? AND a.record_id = ?", array($invoice_found->getFormEntry()->getFormId(), $invoice_found->getFormEntry()->getEntryId()))
             ->andWhere("a.status <> ?", 2)
             ->andWhere("a.payment_amount = ?", $invoice_found->getTotalAmount());
          $transaction = $q->fetchOne();
          ...
          else
          {
              ...
              $transaction->setDateCreated(date("Y-m-d H:i:s"));
              $transaction->setPaymentFullname($fullname);
              $transaction->setPaymentAmount($invoice_found->getTotalAmount());
          ...
          if($transaction_status == 'completed' && $this->invoice_manager->get_invoice_total_owed($invoice_found->getId()) <= 0)
          {
              //Payment is successful
              $invoice_found->setPaid(2);
              $invoice_found->setUpdatedAt(date("Y-m-d H:i:s"));
          }
          elseif($transaction_status == 'failed')
          {
              //Payment has failed
        ...
        error_log("Pesaflow IPN: ".$request_details['status']."/".$this->invoice_manager->get_invoice_total_owed($invoice_found->getId()));
        ...
        elseif($invoice_found->getPaid() == 15)
        {
            $update_details['invoice_status'] = "pending confirmation";
        }
        elseif($invoice_found->getPaid() == 2)
        {
            $update_details['invoice_status'] = "paid";
        }
        elseif($invoice_found->getPaid() == 3)
        {
            $update_details['invoice_status'] = "cancelled";
        }

        $update_details['total_amount'] = $invoice_found->getTotalAmount();
        $update_details['currency'] = $invoice_found->getCurrency();
        $update_details['date_of_invoice'] = $invoice_found->getCreatedAt();
