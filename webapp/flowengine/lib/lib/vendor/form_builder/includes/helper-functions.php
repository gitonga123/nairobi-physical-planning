...
function mf_send_webhook_notification($dbh, $form_id, $entry_id, $webhook_rule_id)
{
	...
	//if webhook URL contain query string, use that whole URL
...
function mf_send_notification($dbh, $form_id, $entry_id, $to_emails, $email_param)
{
	...
	$bcc_emails = $email_param['bcc_emails'];
	...
	$pdf_enable = $email_param['pdf_enable']; //if set to 'true', it will also attach pdf containing the form data
	$pdf_content = $email_param['pdf_content'];
	...
	if (!empty($email_address)) {
		...
		if ($pdf_enable === true) {
			...
			if (!file_exists($mf_settings['upload_dir'] . "/temp")) {
				$old_mask = umask(0);
				mkdir($machform_data_path . $mf_settings['upload_dir'] . "/temp", 0777);
				umask($old_mask);
			...
			if (file_exists($pdf_filename)) {
				$s_message->attach(Swift_Attachment::fromPath($pdf_filename)->setFilename($subject . ".pdf"));
			}
		}
...
function mf_get_settings($dbh)
{
	...
	try {
		...
		$settings['base_url'] = "http{$ssl_suffix}://" . $_SERVER['HTTP_HOST'] . "/forms/";
		return $settings;
...
function mf_get_logic_javascript_block_plot_verification($dbh, $form_id, $page_number)
{
	...
	// Query for plot/block numbers
	$plot_block_query = "
        SELECT element_id, 
            CASE 
                WHEN element_plot_no = 1 THEN 'element_plot_no' 
                WHEN element_block_no = 1 THEN 'element_block_no' 
            END AS element_name 
        FROM " . MF_TABLE_PREFIX . "form_elements 
        WHERE form_id = {$form_id} 
        AND (element_plot_no = 1 OR element_block_no = 1)
    ";

	$plot_no_details = Doctrine_Manager::getInstance()->getCurrentConnection()->fetchAll($plot_block_query);
	...
	foreach ($plot_no_details as $detail) {
		...
		if ($detail['element_name'] == 'element_block_no') {
			$block_number_element_id = "element_{$detail['element_id']}";
		}
	}
	...
	$query = "
        SELECT element_id, 
            CASE 
                WHEN element_owner_phone = 1 THEN 'element_owner_phone'
                WHEN element_owner_address = 1 THEN 'element_owner_address'
                WHEN element_owner_email = 1 THEN 'element_owner_email'
                WHEN element_ward = 1 THEN 'element_ward'
                WHEN element_plot_area = 1 THEN 'element_plot_area'
                WHEN element_plot_length = 1 THEN 'element_plot_length'
                WHEN element_plot_width = 1 THEN 'element_plot_width'
                WHEN element_subcounty = 1 THEN 'element_subcounty'
                WHEN element_ownertype = 1 THEN 'element_ownertype'
            END AS element_name
        FROM " . MF_TABLE_PREFIX . "form_elements 
        WHERE form_id = ? 
        AND (
            element_owner_phone = 1 OR 
            element_owner_address = 1 OR 
            element_owner_email = 1 OR 
            element_ward = 1 OR 
            element_plot_area = 1 OR 
            element_plot_length = 1 OR 
            element_plot_width = 1 OR 
            element_subcounty = 1 OR 
            element_ownertype = 1
        )
    ";
	...
	while ($row = mf_do_fetch_result($sth)) {
		$element_name = substr($row['element_name'], strlen('element_'));
		$element_details[$element_name] = "element_{$row['element_id']}";
	}

	$element_plot_details = json_encode($element_details);

	error_log("Element plot details below ---->");
	error_log($element_plot_details);

	// Logging the element IDs
        ...
        $(document).ready(function() {
			...
			if (plot_details) {
				...
				$.ajax({
					...
					success: function(data) {
						if (data?.success) {
							const plot_details = JSON.parse(data.plot_details);
							const details = plot_details?.details;
			  	...
			  	for (let field in element_to_autofill) {
					if (element_to_autofill.hasOwnProperty(field)) {
						...
						if (field == 'owner_address') {
							let value = details?.physical_address;
							$("#" + element_to_autofill[field]).val(value);
							
						}
						if (field == 'subcounty') {
							let value = details?.sub_county?.title;
							fillSelectElement(value, element_to_autofill[field]);
						}
						if (field == 'owner_phone') {
							let value = details?.related_customer?.phonenumber;
							$("#" + element_to_autofill[field]).val(value);
							
						}
						if (field == 'owner_email') {
							let value = details?.related_customer?.email;
							$("#" + element_to_autofill[field]).val(value);
							
						}
						if (field == 'ward') {
							let value = details?.ward?.title;
							fillSelectElement(value, element_to_autofill[field]);
						}
						if (field == 'plot_area') {
							let value = details?.area;
							$("#" + element_to_autofill[field]).val(value);
			...
			function fillSelectElement(valueName, selectElementId) {
				...
				$('#'+selectElementId + ' option').each(function() {
					if ($(this).text().toLowerCase() === valueName) {
						$('#' + selectElementId).val($(this).val());
...
function mf_get_condition_status_from_input($dbh, $condition_params, $user_input)
{
	...
	if (in_array($element_type, array('text', 'textarea', 'simple_name', 'name', 'simple_name_wmiddle', 'name_wmiddle', 'address', 'phone', 'simple_phone', 'email', 'url'))) {
		...
		$sth = mf_do_query($query, $params, $dbh);
		$row = mf_do_fetch_result($sth);
		if (!empty($row['option_text']) || $row['option_text'] == 0 || $row['option_text'] == '0') {
			$element_value = strtolower($row['option_text']);
		} else {
...
function mf_get_template_variables($dbh, $form_id, $entry_id, $options = array())
{
	...
	$entry_options['show_image_preview'] = $options['show_image_preview'];

	if ($target_is_admin === false) {
		$entry_options['skip_hidden_fields'] = true;
	}
	...
	foreach ($entry_details as $data) {
		...
		if ($data['element_type'] == 'textarea' && !$as_plain_text) {
			...
			if (!$as_plain_text) {
				$template_values[$i] = strip_tags($data['value'], '<a><br/><br><img>');
			} else {
	...
	if (!$as_plain_text) {
		...
		if ($use_list_layout) {
			...
			$content_font_family = 'Lucida Grande,Tahoma,Arial,Verdana';
		...
		foreach ($entry_details as $data) {
			...
			if ($data['element_type'] == 'file') {
				...
				if ($target_is_admin === false) {
					$data['value'] = strip_tags($data['value'], '<a><br/><br><img>');
					$data['value'] = str_replace('&nbsp;', '', $data['value']);
				} else {
					$data['value'] = strip_tags($data['value'], '<a><br/><br><img>');
					$data['value'] = str_replace('&nbsp;', '', $data['value']);
				...
				if (!empty($options['machform_path'])) { //if the form is called from advanced form code
					$data['value'] = '<img src="' . $options['machform_path'] . 'signature_img.php?q=' . $q_string . '" />';
				} else {
		...
		foreach ($ip_whitelist_array as $allowed_ip_address) {
			...
			if ($pos === false) {
				...
				if ($total_ip_parts == 1) {
					...
					if (
						($client_ip_parts[0] == $allowed_ip_parts[0] || $allowed_ip_parts[0] == '*') &&
...
function mf_get_timezone_list()
{
	...
	foreach ($timezone_array as $timezone_name) {
		...
		switch ($timezone_data[$i]['simple_name']) {
			case 'New_York':
				$timezone_data[$i]['simple_name'] = 'Eastern Time (New York)';
				break;
			case 'Chicago':
				$timezone_data[$i]['simple_name'] = 'Central Time (Chicago)';
				break;
			case 'Denver':
				$timezone_data[$i]['simple_name'] = 'Mountain Time (Denver)';
				break;
			case 'Phoenix':
				$timezone_data[$i]['simple_name'] = 'Mountain Time - no DST (Phoenix)';
				break;
			case 'Los_Angeles':
				$timezone_data[$i]['simple_name'] = 'Pacific Time (Los Angeles)';
				break;
			case 'Anchorage':
				$timezone_data[$i]['simple_name'] = 'Alaska (Anchorage)';
				break;
			case 'Adak':
				$timezone_data[$i]['simple_name'] = 'Hawaii (Adak)';
				break;
			case 'Honolulu':
				$timezone_data[$i]['simple_name'] = 'Hawaii - no DST (Honolulu)';
				break;
		}
