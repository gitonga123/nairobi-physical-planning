...
function mf_display_select($element)
{
	...
	$attr_readonly = '';
	if ($element->is_readonly) {
		$attr_readonly = ' disabled readonly="readonly" style="background-color: #e9ecef; opacity: 1;"';
	}
	$element_markup = <<<EOT
...
function mf_display_form($dbh, $form_id, $form_params = [])
{
	...
	foreach ($element as $element_data) {
		...
		// TODO: if any errors re update this
		// else {
		// 	$element_data->is_readonly = "";
		// }
		//OTB END
...
function mf_display_form_payment($dbh, $form_id, $record_id, $form_params = array())
{
	...
	if ($_SESSION['mf_form_payment_access'][$form_id] !== true) {
		return "Your session has been expired. Please <a href='/forms/view?id={$form_id}'>click here</a> to start again.";
	}
