...
function mf_validate_unique($value)
{
	...
	if (!empty($_SESSION['edit_entry']) && ($_SESSION['edit_entry']['form_id'] == $form_id)) {
		...
		$query = "select count($element_name) total from " . MF_TABLE_PREFIX . "form_{$form_id} where {$element_name}=? and {$element_name} <> '' and `id` != ? and {$element_name} is not null and {$element_name} <> '' and `status` = 1";
		$params = array($input_value, $_SESSION['edit_entry']['entry_id']);

		$sth = mf_do_query($query, $params, $dbh);
		$row = mf_do_fetch_result($sth);
	} else {
		$query = "select count($element_name) total from " . MF_TABLE_PREFIX . "form_{$form_id} where {$element_name}=? and {$element_name} <> '' and resume_key is null and `status`= 1";
...
//remote validation for values
function mf_validate_remote_url($value)
{
	global $mf_lang;
	...
	if (empty($input_value) && $input_value != 0) //Blank value shouldn't return an error. That job is left to the required validator.
	{
		return true;
	}
	...
	$remote_url  = $exploded[0]; //http://remote
	$criteria = $exploded[1]; //can be 'records', 'norecords' or 'value'
	$result_value = $exploded[2]; //If criteria is 'value'
	$remote_username = $exploded[3]; //Remote username
	$remote_password = $exploded[4]; //Remote password
	$form_id = $exploded[5]; //Remote password
	$remote_post = $exploded[6]; //Remote password
	...
	if ($pos === false) {
		...
		$remote_url = str_replace('$value', curl_escape($ch, $input_value), $remote_url);
	}

	$pos = strpos($remote_post, '$value');

	if ($pos === false) {
		//dont' do anything
	} else {
		$remote_post = str_replace('$value', curl_escape($ch, $input_value), $remote_post);
	}

	error_log("Updates Manager: Call in mf_validate: " . $remote_url);
	...
	if ($validation_result['test'] == false) {
...
//OTB ADD
//validation for unique checking on db table
function mf_validate_plot_no($value)
{
	global $mf_lang;

	$input_value  = trim($value[0]);

	$dbh = $value[2]['dbh'];

	$query = "select count(*) from plot where plot_no like ? and plot_status = ?";
	$params = array('%' . $input_value . '%', 'Black-Listed');
	...
	if (!empty($row['count(*)'])) {
		return sprintf($mf_lang['val_plot_no'], $value[0]);
	} else {
		return true;
	}
}
...
function mf_validate_element($input, $rules)
{
	...
	foreach ($input as $key => $value) {
		...
		if (($error_message || is_array($error_message)) && count($error_message) > 0) {
