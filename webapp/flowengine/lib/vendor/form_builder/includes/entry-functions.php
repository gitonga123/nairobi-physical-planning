...
$query = "select 
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		$form_elements[$i]['element_footprint'] = $row['element_footprint']; //OTB patch - Element footprint
		$form_elements[$i]['element_plotsize'] = $row['element_plotsize']; //OTB patch
		$form_elements[$i]['element_actualplotratio'] = $row['element_actualplotratio']; //OTB patch
		$form_elements[$i]['element_permittedgroundcoverage'] = $row['element_permittedgroundcoverage']; //OTB patch
		$form_elements[$i]['element_zone'] = $row['element_zone']; //OTB patch
		$form_elements[$i]['element_projectcost'] = $row['element_projectcost']; //OTB patch
		$form_elements[$i]['element_grandtotalplintharea'] = $row['element_grandtotalplintharea']; //OTB patch
		$form_elements[$i]['element_groundcoveragereason'] = $row['element_groundcoveragereason']; //OTB patch
		$form_elements[$i]['element_plotratioreason'] = $row['element_plotratioreason']; //OTB patch
		...
		$form_elements[$i]['element_field_name'] = $row['element_field_name']; //OTB patch
		$form_elements[$i]['element_option_query'] = $row['element_option_query']; //OTB patch
		$form_elements[$i]['element_existing_form'] = $row['element_existing_form']; //OTB patch
		$form_elements[$i]['element_existing_stage'] = $row['element_existing_stage']; //OTB patch
		$form_elements[$i]['element_select_options'] = $row['element_select_options']; //OTB patch
	...
	$query = "select element_id,option_id,`option_text` from " . MF_TABLE_PREFIX . "element_options where form_id=? and live=1";
	$params = array($form_id);
	...
	foreach ($form_elements as $element) {
		...
		$element_footprint = $element['element_footprint']; // OTB patch Element footprint
		$element_plotsize = $element['element_plotsize']; // OTB patch
		$element_actualplotratio = $element['element_actualplotratio']; // OTB patch
		$element_permittedgroundcoverage = $element['element_permittedgroundcoverage']; // OTB patch
		$element_zone = $element['element_zone']; // OTB patch
		$element_projectcost = $element['element_projectcost']; // OTB patch
		$element_grandtotalplintharea = $element['element_grandtotalplintharea']; // OTB patch
		$element_groundcoveragereason = $element['element_groundcoveragereason']; // OTB patch
		$element_plotratioreason = $element['element_plotratioreason']; // OTB patch
		...
		$element_block_no = $element['element_block_no']; // OTB patch
		$element_subcounty = $element['element_subcounty']; // OTB patch
		$element_owner_phone = $element['element_owner_phone'];
		$element_owner_address = $element['element_owner_address'];
		$element_owner_email = $element['element_owner_email'];
		$element_ward = $element['element_ward'];
		$element_plot_area = $element['element_plot_area'];
		$element_plot_length = $element['element_plot_length'];
		$element_plot_width = $element['element_plot_width'];
		...
		$element_field_name = $element['element_field_name']; // OTB patch
		$element_option_query = $element['element_option_query']; // OTB patch
		$element_existing_form = $element['element_existing_form']; // OTB patch
		$element_existing_form = $element['element_existing_stage']; // OTB patch
		$element_select_options = $element['element_select_options']; // OTB patch
		...
		if ('simple_name' == $element_type) { //Simple Name - 2 elements
			...
			if (!empty($filename_array)) {
				...
				foreach ($filename_array as $filename_value) {
					...
					//OTB ADD
					$arr_filename = explode('_', $filename_value);
					$save_filename = substr($arr_filename[2], 0, strpos($arr_filename[2], '-'));
					$file_1 = substr($filename_value, strpos($filename_value, '-') + 1);
					$filename_value = substr($file_1, strpos($file_1, '-') + 1);

					$form_values['element_' . $element_id]['default_value'][$i]['saved_filename'] = $save_filename;
		...
		else if ('plinth_area' == $element_type) { //Plinth Area - 2 elements
			$form_values['element_' . $element_id . '_1']['default_value'] = $entry_data['element_' . $element_id . '_1'];
			$form_values['element_' . $element_id . '_2']['default_value'] = $entry_data['element_' . $element_id . '_2'];
		} // OTB patch - Total plinth area
		else if ('total_plinth_area' == $element_type) { //Total plinth area - 2 elements
			$form_values['element_' . $element_id . '_1']['default_value'] = $entry_data['element_' . $element_id . '_1'];
			$form_values['element_' . $element_id . '_2']['default_value'] = $entry_data['element_' . $element_id . '_2'];
...
function mf_get_entry_details($dbh, $form_id, $entry_id, $options = array())
{
	...
	//hide admin and hidden fields
	if (!empty($options['review_mode']) || !empty($options['skip_hidden_fields'])) {
	...
	if (!empty($options['checkbox_image'])) {
		...
		$checkbox_image = '/form_builder/images/icons/checkbox_16.gif';
	}

	//$show_image_preview = $options['show_image_preview']; //if set to 'true', the uploaded image files will have a preview being displayed
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		$form_elements[$i]['element_footprint'] = $row['element_footprint']; //OTB patch - for footprint
		$form_elements[$i]['element_plotsize'] = $row['element_plotsize']; //OTB patch
		$form_elements[$i]['element_actualplotratio'] = $row['element_actualplotratio']; //OTB patch
		$form_elements[$i]['element_permittedgroundcoverage'] = $row['element_permittedgroundcoverage']; //OTB patch
		$form_elements[$i]['element_zone'] = $row['element_zone']; //OTB patch
		...
		if ($trans_text) {
			$form_elements[$i]['element_guidelines'] = $trans_text;
		}
		...
		$form_elements[$i]['element_mark_file_with_qr_code'] = $row['element_mark_file_with_qr_code'];//OTB Addition QR on attachments
		$form_elements[$i]['element_file_qr_all_pages'] = $row['element_file_qr_all_pages'];//OTB Addition QR on attachments
		$form_elements[$i]['element_file_qr_page_position'] = $row['element_file_qr_page_position'];//OTB Addition QR on attachments
		$form_elements[$i]['element_file_qr_users'] = $row['element_file_qr_users'];//OTB Addition QR on attachments
		$form_elements[$i]['element_grandtotalplintharea'] = $row['element_grandtotalplintharea'];//OTB Addition QR on attachments
		$form_elements[$i]['element_select_options'] = $row['element_select_options'];//OTB select options
		$form_elements[$i]['element_table_name'] = $row['element_table_name'];//OTB select options
		$form_elements[$i]['element_field_value'] = $row['element_field_value'];//OTB select options
		$form_elements[$i]['element_field_name'] = $row['element_field_name'];//OTB select options
		$form_elements[$i]['element_option_query'] = $row['element_option_query'];//OTB select options
		$trans_text = $translation->getOptionTranslationManual("ap_form_elements", "element_title", $form_id, $row['element_id'], $locale);
		if ($trans_text) {
			$element_title_lookup[$row['element_id']] = $trans_text;
		}
	...
	//get form element options
	$query = "select aeo_id, element_id,option_id,`option_text` from " . MF_TABLE_PREFIX . "element_options where form_id=? and live=1 order by position asc";
	$params = array($form_id);
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		$element_option_lookup[$element_id][$option_id] = $row['option_text']; //array index will hold option_id

		$trans_text = $translation->getOptionTranslationManual("ap_element_options", "option_text", $row['aeo_id'], $option_id, $locale);
		if ($trans_text) {
			$element_option_lookup[$element_id][$option_id] = $trans_text;
		}
	}
	...
	$query = "select 
						A.element_id,
						A.option_id,
						(select if(B.element_matrix_parent_id=0,A.option_text,
							(select 
									C.`option_text` 
							   from 
	...
	foreach ($form_elements as $element) {
		...
		$element_matrix_allow_multiselect = $element['element_matrix_allow_multiselect'];
		$element_footprint = $element['element_footprint'];
		$element_plotsize = $element['element_plotsize'];
		$element_actualplotratio = $element['element_actualplotratio'];
		$element_permittedgroundcoverage = $element['element_permittedgroundcoverage'];
		$element_zone = $element['element_zone'];
		$element_projectcost = $element['element_projectcost'];
		$element_groundcoveragereason = $element['element_groundcoveragereason'];
		$element_grandtotalplintharea = $element['element_grandtotalplintharea'];
		$element_plotratioreason = $element['element_plotratioreason'];
		$element_permitteduser = $element['element_permitteduser'];
		$element_mark_file_with_qr_code = $element['element_mark_file_with_qr_code'];//OTB Add QR on attachments
		$element_file_qr_all_pages = $element['element_file_qr_all_pages'];//OTB Add QR on attachments
		$element_file_qr_page_position = $element['element_file_qr_page_position'];//OTB Add QR on attachments
		...
		$element_block_no = $element['element_block_no'];//OTB Add QR on attachments
		...
		$element_existing_stage = $element['element_existing_stage'];
		$element_table_name = $element['element_table_name'];
		$element_field_value = $element['element_field_value'];
		$element_field_name = $element['element_field_name'];
		$element_option_query = $element['element_option_query'];
		...
		$entry_details[$i]['element_footprint'] = $element_footprint;
		$entry_details[$i]['element_plotsize'] = $element_plotsize;
		$entry_details[$i]['element_actualplotratio'] = $element_actualplotratio;
		$entry_details[$i]['element_permittedgroundcoverage'] = $element_permittedgroundcoverage;
		$entry_details[$i]['element_zone'] = $element_zone;
		$entry_details[$i]['element_projectcost'] = $element_projectcost;
		$entry_details[$i]['element_groundcoveragereason'] = $element_groundcoveragereason;
		$entry_details[$i]['element_grandtotalplintharea'] = $element_grandtotalplintharea;
		$entry_details[$i]['element_plotratioreason'] = $element_plotratioreason;
		$entry_details[$i]['element_permitteduser'] = $element_permitteduser;
		$entry_details[$i]['element_mark_file_with_qr_code'] = $element_mark_file_with_qr_code;
		$entry_details[$i]['element_file_qr_all_pages'] = $element_file_qr_all_pages;
		$entry_details[$i]['element_file_qr_page_position'] = $element_file_qr_page_position;
		...
		$entry_details[$i]['element_block_no'] = $element_block_no;
		$entry_details[$i]['element_subcounty'] = $element_subcounty;
		$entry_details[$i]['element_owner_phone'] = $element_owner_phone;
		$entry_details[$i]['element_owner_address'] = $element_owner_address;
		$entry_details[$i]['element_owner_email'] = $element_owner_email;
		$entry_details[$i]['element_ward'] = $element_ward;
		$entry_details[$i]['element_plot_area'] = $element_plot_area;
		$entry_details[$i]['element_plot_length'] = $element_plot_length;
		$entry_details[$i]['element_plot_width'] = $element_plot_width;
		...
		$entry_details[$i]['element_field_value'] = $element_field_value;
		$entry_details[$i]['element_field_name'] = $element_field_name;
		...
		if ('simple_name' == $element_type) { //Simple Name - 2 elements
			...
			if (!empty($filename_array)) {
				...
				$my_allowed_signing_fields = Functions::fields_i_can_sign_for_this_form($form_id);
				foreach ($filename_array as $filename_value) {
					$actual_file_name = $filename_value;
					$filename_md5 = md5($filename_value);
					$ori_filename = $filename_value;
					$filename_path = $options['machform_data_path'] . $mf_settings['upload_dir'] . "/form_{$form_id}/files/{$filename_value}.tmp";
					if (!file_exists($filename_path)) {
						$filename_path = $options['machform_data_path'] . $mf_settings['upload_dir'] . "/form_{$form_id}/files/{$filename_value}";
					}
					...
					if ($element_mark_file_with_qr_code == 1 and $element_file_qr_users == "clients" and empty(sfContext::getInstance()->getUser()->getAttribute('userid'))) {
						...
						$q_string = base64_encode("form_id={$form_id}&id={$entry_id}&el=element_{$element_id}&element_mark_file_with_qr_code={$element_mark_file_with_qr_code}&element_file_qr_all_pages={$element_file_qr_all_pages}&element_file_qr_page_position={$element_file_qr_page_position}&hash={$filename_md5}");
					} else if ($element_mark_file_with_qr_code == 1 and $element_file_qr_users == "both") {
						$q_string = base64_encode("form_id={$form_id}&id={$entry_id}&el=element_{$element_id}&element_mark_file_with_qr_code={$element_mark_file_with_qr_code}&element_file_qr_all_pages={$element_file_qr_all_pages}&element_file_qr_page_position={$element_file_qr_page_position}&hash={$filename_md5}");
					} else {
						$q_string = base64_encode("form_id={$form_id}&id={$entry_id}&el=element_{$element_id}&hash={$filename_md5}");
					}
					//OTB Africa End Add QR on attachments
					...
					if (!empty($options['show_attach_image'])) {
						...
						$entry_details[$i]['value'] .= '<img src="/form_builder/images/icons/185.png" align="absmiddle" style="vertical-align: middle" />&nbsp;' . "{$filename_value} {$file_size}<br/>";
					} else {
						$filename_value = htmlspecialchars($filename_value);
						...
						if (!empty($options['strip_download_link'])) {
							...
							$file_extension = strtolower(substr(strrchr($filename_value, "."), 1));
							if (in_array($file_extension, array('bmp', 'gif', 'jpg', 'jpe', 'jpeg', 'png'))) {
								$is_image_file = true;
							} else {
								$is_image_file = false;
							}
							...
							if ($show_image_preview === true && $is_image_file) {
								if (!empty($options['machform_path'])) { //if the form is called from advanced form code
									$entry_details[$i]['value'] .= '<img src="/plan/forms/viewimg?q=' . $q_string . '" style="max-width: 350px;margin: 15px 0;" /><br/><a class="entry_link" href="https://sisibo.uasingishu.go.ke/plan/forms/download?q=' . $q_string . '">' . $filename_value . '</a><br/>';
								} else {
									$entry_details[$i]['value'] .= '<img src="https://sisibo.uasingishu.go.ke' . '/plan/forms/viewimg?q=' . $q_string . '" style="max-width: 350px;margin: 15px 0;" /><br/><a class="entry_link" href="https://sisibo.uasingishu.go.ke' . '/plan/forms/download?q=' . $q_string . '">' . $filename_value . '</a><br/>';
								}
							} else {
								if (!empty($options['machform_path'])) { //if the form is called from advanced form code
									$entry_details[$i]['value'] .= '<a class="entry_link" href="https://sisibo.uasingishu.go.ke/plan/forms/download?q=' . $q_string . '">' . $filename_value . '</a><br/>';
								} else {
									...
									$_is_signed = file_exists($signed_file_path);
									...
									if (!$_is_signed) {
										...
										$entry_details[$i]['value'] .= '<a class="entry_link btn" href="https://sisibo.uasingishu.go.ke' . '/' . $signed_file_path . '" download="' . $filename_value . '">' . str_replace('.pdf', '--signed.pdf', $filename_value) . '</a><br/>';
									}
								}
							}
						}
					...
					//IFC FILE VIEWER
					//ONLY FOR BACKEND
					if ($entry_details[$i]['element_file_ifc'] && !empty(sfContext::getInstance()->getUser()->getAttribute('userid'))) {
						//check status if updated
						$q_ifc = "SELECT * FROM ifc_file WHERE application LIKE ?";
						$params = array($ori_filename);

						$fetch_q = mf_do_query($q_ifc, $params, $dbh);
						$ifc_files = mf_do_fetch_result($fetch_q);
						if ($ifc_files['status'] === 'ok') {
							$entry_details[$i]['value'] .= '<button type="button" class="btn btn-primary ifc-button" data-toggle="modal" data-target="#ifc_viewer" data-application="' . $ori_filename . '">Render</button><br/>';
						} elseif ($ifc_files['status'] === 'error' && strlen($ifc_files['message'])) {
							$entry_details[$i]['value'] .= 'IFC ERROR: ' . $ifc_files['message'] . '<br/>';
						}
					}
					$j++;
				}
			...
			if (!empty($entry_data['element_' . $element_id])) {
				if ($element_select_options == "table") {
					$query = "SELECT {$element_field_value}, {$element_field_name} FROM {$element_table_name} WHERE {$element_field_value} = ?";
					$params = array($entry_data['element_' . $element_id]);

					$sth = mf_do_query($query, $params, $dbh);
					$app_row = mf_do_fetch_result($sth);
					error_log('--------TABLE QUERY------');
					error_log(print_r($app_row, true));
					if ($app_row) {
						$entry_details[$i]['value'] = $app_row[$element_field_name];
					}
				} elseif ($element_select_options == 'query') {
					$query = $element_option_query;

					//$sth = mf_do_query($query, $params, $dbh);
					$sth = mf_do_query($query, [], $dbh);
					while ($app_row = mf_do_fetch_result($sth)) {
						error_log('--------QUERY------');
						error_log(print_r($app_row, true));
						if ($app_row[$element_field_value] == $entry_data['element_' . $element_id]) {
							$entry_details[$i]['value'] = $app_row[$element_field_name];
						}
					}
				} elseif ($element_select_options == 'application') {
					$query = "SELECT id, application_id FROM form_entry WHERE id = ?";
					$params = [$entry_data['element_' . $element_id]];

					$sth = mf_do_query($query, $params, $dbh);
					$app_row = mf_do_fetch_result($sth);
					error_log('--------Application------');
					error_log(print_r($app_row, true));
					$entry_details[$i]['value'] = $app_row['application_id'];
				} else {
			...
			//if this is the first row of the matrix, get the main matrix title and put as section break
			if (!empty($element_guidelines) && !empty($element_constraint)) {

				$entry_details[$i]['label'] = $element_guidelines;
				$entry_details[$i]['element_type'] = 'section';
				...
				$entry_details[$i]['element_id'] = $entry_details[$i - 1]['element_id'];
				unset($entry_details[$i - 1]['element_id']);
			}
		...
		} else if ('plinth_area' == $element_type) {
			$plinth_area_value = '<table class="table"><tr><th>New SM</th><th>Existing SM</th></tr>';
			$plinth_area_value .= '<tr><td>' . trim($entry_data['element_' . $element_id . '_1']) . '</td><td>' . trim($entry_data['element_' . $element_id . '_2']) . '<td></tr>';
			$plinth_area_value .= '</table>';
			if (!empty($plinth_area_value)) {
				$entry_details[$i]['value'] = $plinth_area_value;
			}
		}//OTB patch - total plinth area
		else if ('total_plinth_area' == $element_type) {
			$total_plinth_area = '<table class="table"><tr><th>Total New</th><th>Total Existing</th></tr>';
			$total_plinth_area .= '<tr><td>' . trim($entry_data['element_' . $element_id . '_1']) . '</td><td>' . trim($entry_data['element_' . $element_id . '_2']) . '</td></tr>';
			$total_plinth_area .= '</table>';
			if (!empty($total_plinth_area)) {
				$entry_details[$i]['value'] = $total_plinth_area;
			}
		} else { //element with only 1 input
			if (isset($entry_data['element_' . $element_id])) {
...
function mf_display_entries_table($dbh, $form_id, $options)
{
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		$element_option_lookup[$element_id][$option_id] = htmlspecialchars(strip_tags($row['option_text']), ENT_QUOTES);
	}
			...
			if (in_array($filter_element_type, array('radio', 'select', 'matrix_radio'))) {
				...
				$query = "select 
							   ...
							   where 
							   		...
							   		element_id=? and
							   		live=1 and 
							   		`option_text` {$where_operand} {$where_keyword}";
...
function mf_get_filtered_entries_ids($dbh, $form_id, $options = array())
{
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		$element_option_lookup[$element_id][$option_id] = htmlspecialchars($row['option_text'], ENT_QUOTES);
	}
