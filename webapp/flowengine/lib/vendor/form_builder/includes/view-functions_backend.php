...
function mf_display_text($element)
{
	...
	if (!empty($element->is_error)) {
		...
		if ($element->error_message != 'error_no_display') {
			if ($element->field_error_message) {
				$error_message = "<p class=\"error\">{$element->field_error_message}</p>";
			} else {
	...
	/**
	 * OTB patch check either this field is set to be used as 
	 * plot size
	 * actual plot ratio
	 * actual ground coverage
	 */
	//variable to hold identifier for  plot size or actual plot ratio or actual ground coverage
	$myidentifier = null;
	$warning = null;
	$attribute_disabled = null;
	//default value
	$default_value = $element->default_value;
	//for unit of measure in hectares to square meters
	$hect_sqrmtr = null;
	//button for actual plot ratio or ground coverage
	$btn_actualgroundcoverage_or_actualplotratio = null;
	/////
	...
	if (!empty($element->plotsize)) {
		//error_log("this element is used as a_plotsize >>>>".$element->id) ;
		$myidentifier = "a_plotsize";
		//set default value 
		// $default_value = 0 ;
		//for plot size only
		$hect_sqrmtr = "<label for='hect_sqrmt'> Unit of Measure</label><select id='hect_sqrmt'> <option value='none'> Select Unit </option> <option value='sqr'> sqr meters </option> <option value='hect'> hectares </option></select> <div class='help'> You can enter plot size in hectares and select hectares as unit of measure. The system will convert the hectares to square meters </div> ";
	} else if (!empty($element->projectcost)) {
		$myidentifier = "a_projectcost";
		//set default value 
		//$default_value = 0 ;
	} else if (!empty($element->actualplotratio)) {
		//set default value 
		//$default_value = 0 ;
		$myidentifier = "a_actualplotratio";
		$attribute_disabled = "readonly";
		$warning = "    
			<div id='warning_plotratio_exceeded' class='alert alert-danger'> 
		...
		//set default value 
		//$default_value = 0 ;   
		$myidentifier = "a_actualgroundcoverage";
		$attribute_disabled = "readonly";
		$warning = "    
...
function mf_display_textarea($element)
{
	...
	//OTB patch 
	//Check if element should be used for inluding reasons for exceeding permitted plot ratio and ground coverage.
	$custom_property_class = null;
	if (!empty($element->groundcoveragereason)) {
		$custom_property_class = "a_groundcoveragereason";
	} else if (!empty($element->plotratioreason)) {
		$custom_property_class = "a_plotratioreason";
	}
	$element_markup = <<<EOT
		<li id="li_{$element->id}" {$li_style} {$li_class}  class="form-group">
		<label class="description col-sm-2 control-label" for="element_{$element->id}">{$element->title} {$span_required}</label>
...
function mf_display_signature($element)
{
	...
	if (!empty($element->is_design_mode)) {
		...
		$signature_markup = <<<EOT
	        <div class="mf_sig_wrapper {$element->size}" {$element->edit_style}>
...
function mf_display_file($element)
{
	...
	if (!empty($element->populated_value['element_' . $element->id]['default_value'])) {
		foreach ($element->populated_value['element_' . $element->id]['default_value'] as $data) {
			...
			//OTB hide delete button if readonly
			$readonly_remove = '';
			if (!$element->is_readonly) {
				$readonly_remove = "<div class=\"cancel\">
					<a href=\"javascript:remove_attachment('{$data['filename']}',{$element->form_id},{$element->id},'{$queue_id}',{$db_live_status},{$data['entry_id']},'{$data['saved_filename']}');\"><img border=\"0\" src=\"/form_builder/images/icons/delete.png\"></a>
					</div>";
			}
			$queue_content .= <<<EOT
						...
						{$readonly_remove}
						<span class="fileName">
						  <img align="absmiddle" src="/form_builder/images/icons/attach.gif" class="file_attached">{$display_filename} {$data['filesize']}
						</span>
						</div>
EOT;
		}
			...
			if (file_exists($listfile_name)) {
				...
				foreach ($uploaded_files as $tmp_filename_path) {
					...
					$queue_content .= <<<EOT
							<div class="uploadifyQueueItem completed" id="{$queue_id}">
							<div class="cancel">
							<a href="javascript:remove_attachment('{$filename_value}',{$element->form_id},{$element->id},'{$queue_id}',0,'{$file_token}');"><img border="0" src="/form_builder/images/icons/delete.png"></a>
						    </div>
							<span class="fileName">
...
EOT;
		...
		if (!empty($element->file_type_list) && !empty($element->file_enable_type_limit)) {
			if ($element->file_block_or_allow == 'a') { //if this is an allow list
				...
				array_walk($allowed_file_types, fn(&$val) => $val = "*." . strtolower(trim($val)));
		 ...
		 if(is_support_html5_uploader() && $('#element_{$element->id}').length > 0){
		 	$('#element_{$element->id}').uploadifive({
				...
				'onUploadComplete' : function(file, response) {
					...
					if(is_valid_response == true && response_json.status == "ok"){
						var remove_link = "<a class=\"close\" href=\"javascript:remove_attachment('" + response_json.message + "',{$element->form_id},{$element->id},'" + queue_item_id + "',0,'{$file_token}');\"><img border=\"0\" src=\"/form_builder/images/icons/delete.png\" /></a>";
			        	...
			        	$("#" + queue_item_id + " a.close").replaceWith('<img style="float: right" border="0" src="/form_builder/images/icons/exclamation.png" />');
			...
			$("#element_{$element->id}_upload_link").remove();
		      ...
		      $('#element_{$element->id}').uploadify({
		        ...
		        'queueID'	  : 'element_{$element->id}_queue',
		        {$size_limit}
		        'buttonImg'   : '/form_builder/images/upload_button.png',
				...
				'onComplete'  : function(event, ID, fileObj, response, data) {
					...
					if(is_valid_response == true && response_json.status == "ok"){
						var remove_link = "<a href=\"javascript:remove_attachment('" + response_json.message + "',{$element->form_id},{$element->id},'element_{$element->id}" + ID + "',0,'{$file_token}');\"><img border=\"0\" src=\"/form_builder/images/icons/delete.png\" /></a>";
			        	...
			        	$("#element_{$element->id}" + ID + " div.cancel > a ").replaceWith('<img border="0" src="/form_builder/images/icons/exclamation.png" />');
...
function mf_display_phone($element)
{
	...
	$attr_readonly = '';
	$guidelines = '';
	$el_class = array('phone form-group');
	...
	$element_markup = <<<EOT
		...
		<span class="phone_1 col-sm-3">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element form-control text" size="3" maxlength="3" value="{$default_value_1}" type="text" {$element->edit_style}/> -
			<label for="element_{$element->id}_1">###</label>
		</span>
		<span class="phone_2 col-sm-3">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} class="element form-control text" size="3" maxlength="3" value="{$default_value_2}" type="text" {$element->edit_style}/> -
			<label for="element_{$element->id}_2">###</label>
		</span>
		<span class="phone_3 col-sm-3">
	 		<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} class="element form-control text" size="4" maxlength="4" value="{$default_value_3}" type="text" />
...
function mf_display_date($element)
{
	...
	$attr_readonly = '';
	$guidelines = '';
	$el_class = array('date_field form-group');
...
EOT;
	...
	$element_markup = <<<EOT
		...
		<span class="description col-sm-2 control-label">{$element->title} {$span_required}</span>
		<span class="date_mm col-sm-2">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element form-control text" size="2" maxlength="2" value="{$default_value_1}" type="text"/> /
			<label for="element_{$element->id}_1">{$mf_lang['date_mm']}</label>
		</span>
		<span class="date_dd col-sm-2">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} class="element form-control text" size="2" maxlength="2" value="{$default_value_2}" type="text" /> /
			<label for="element_{$element->id}_2">{$mf_lang['date_dd']}</label>
		</span>
		<span class="date_yyyy col-sm-2">
	 		<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} class="element form-control text" size="4" maxlength="4" value="{$default_value_3}" type="text" />
			<label for="element_{$element->id}_3">{$mf_lang['date_yyyy']}</label>
		</span>
		<span id="calendar_{$element->id}" class="col-sm-2">
		    <input type="hidden" value="" name="element_{$element->id}_datepick" id="element_{$element->id}_datepick">
			<div style="{$cal_img_style}"><span style="font-size:20px;" id="cal_img_{$element->id}" class="fa fa-calendar datepicker" alt="Pick a date." ></span></div>
		</span>
		{$calendar_script}
		<span class="col-sm-10 col-sm-offset-2">{$guidelines} {$error_message}</span>
		</li>
...
EOT;
	...
	$element_markup = <<<EOT
		...
		<span id="calendar_{$element->id}">
			<input type="hidden" value="" name="element_{$element->id}_datepick" id="element_{$element->id}_datepick">
			<div style="{$cal_img_style}"><img id="cal_img_{$element->id}" class="datepicker" src="/form_builder/images/calendar.gif" alt="Pick a date." /></div>
		</span>
		{$calendar_script}
		<span class="col-sm-10 col-sm-offset-2">{$guidelines} {$error_message}</span>
		</li>
...
function mf_display_radio($element)
{
	...
	$span_required = '';
	$guidelines = '';
	$el_class = array('multiple_choice form-group');
	...
	$disable_script = "";
	if ($element->is_readonly) {
		$disable_script = "<script> $(function(){ $('#li_{$element->id} fieldset :radio:not(:checked)').attr('disabled', true); }); </script>";
	}
...
function mf_display_checkbox($element)
{
	...
	$span_required = '';
	$guidelines = '';
	$el_class = array('checkboxes form-group');
	...
	foreach ($element->options as $option) {
		...
		$pre_option_markup = '';
		$pre_option_markup .= "<input id=\"element_{$element->id}_{$option->id}\" {$price_definition_data_attr} name=\"element_{$element->id}_{$option->id}\" class=\"element checkbox\" type=\"checkbox\" value=\"1\" {$checked} {$element->is_readonly}/>\n";
		$pre_option_markup .= "<label class=\"choice\" for=\"element_{$element->id}_{$option->id}\">{$option->option}</label>\n";
	...
	if (!empty($element->choice_has_other)) {
		...
		$pre_option_markup = '';
		$pre_option_markup .= "<input id=\"element_{$element->id}_0\" name=\"element_{$element->id}\" class=\"element checkbox\" onchange=\"clear_cb_other(this,{$element->id});\"  type=\"checkbox\" value=\"\" {$checked} {$element->is_readonly}/>\n";
		$pre_option_markup .= "<label class=\"choice other\" for=\"element_{$element->id}_0\">{$element->choice_other_label}</label>\n";
		$pre_option_markup .= "<label style=\"display: none\" for=\"element_{$element->id}_other\">{$element->choice_other_label}</label>\n";
		$pre_option_markup .= "<input type=\"text\" value=\"{$other_value}\" class=\"element text other\" name=\"element_{$element->id}_other\" id=\"element_{$element->id}_other\" onclick=\"\$('#element_{$element->id}_0').prop('checked',true);\" {$element->is_readonly}/>\n";
...
function mf_display_select($element)
{
	...
	$span_required = '';
	$guidelines = '';
	$el_class = array('dropdown form-group');
	...
	foreach ($element->options as $option) {
		...
		//don't display the option if the total entries for the particular option has reached the limit
		if (!$element->is_design_mode && !$element->is_edit_entry && !empty($element->choice_max_entry)) {
			$query = "SELECT COUNT(*) as total_entry FROM " . MF_TABLE_PREFIX . "form_{$element->form_id} where element_{$element->id} = {$option->id} and `status` = 1";

			$row = Doctrine_Manager::getInstance()->getCurrentConnection()->fetchAssoc($query);
			if ($row[0]['total_entry'] >= $element->choice_max_entry) {
				continue;
			}
		}
	...
	//check if element has table source set
	if ($element->select_options == "table") {
		$table_name = $element->table_name;
		$option_value = $element->field_value;
		$option_name = $element->field_name;

		$query = "SELECT {$option_value}, {$option_name} FROM {$table_name} ORDER BY {$option_name} ASC";
		$table_rows = Doctrine_Manager::getInstance()->getCurrentConnection()->fetchAssoc($query);

		$option_markup = "";
		foreach ($table_rows as $option) {
			if ($element->populated_value['element_' . $element->id]['default_value'] == $option[$option_value]) {
				$option_markup .= "<option value='" . $option[$option_value] . "' selected='selected'>" . $option[$option_name] . "</option>";
			} else {
				$option_markup .= "<option value='" . $option[$option_value] . "'>" . $option[$option_name] . "</option>";
			}
		}
	}
	if ($element->select_options == "query") {
		$query_rows = Doctrine_Manager::getInstance()->getCurrentConnection()->fetchAssoc($element->option_query);

		$option_markup = "";
		$option_markup .= "<option value=''></option>";

		foreach ($query_rows as $option) {
			if ($element->populated_value['element_' . $element->id]['default_value'] == $option[$element->field_value]) {
				$option_markup .= "<option value='" . $option[$element->field_value] . "' selected='selected'>" . $option[$element->field_name] . "</option>";
			} else {
				$option_markup .= "<option value='" . $option[$element->field_value] . "'>" . $option[$element->field_name] . "</option>";
			}
		}
	}
	if ($element->select_options == "application") {
		$sql = "";
		if ($_SESSION['user_id']) //if a reviewer is submitting from the backend, use a different session variable
		{
			$sql = "SELECT a.id, a.application_id FROM form_entry a INNER JOIN form_entry_shares s ON s.formentryid = .id WHERE a.form_id = {$element->existing_form} AND (a.user_id = {$_SESSION['user_id']} OR s.receiverid = {$_SESSION['user_id']}) AND a.approved = {$element->existing_stage} AND a.parent_submission = 0 AND a.deleted_status = 0";
		} else {
			$sql = "SELECT a.id as id, a.application_id as application_id FROM form_entry a WHERE a.form_id = {$element->existing_form} AND a.approved = {$element->existing_stage} AND a.parent_submission = 0 AND a.deleted_status = 0";
		}
		error_log(var_export($element, true));
		error_log('-------SQL-------' . $sql);
		$app_rows = Doctrine_Manager::getInstance()->getCurrentConnection()->fetchAssoc($sql);
		$option_markup = "";
		foreach ($app_rows as $option) {
			if ($element->populated_value['element_' . $element->id]['default_value'] == $option["id"]) {
				$option_markup .= "<option value='" . $option["id"] . "' selected='selected'>" . $option["application_id"] . "</option>";
			} else {
	...
	$q = Doctrine_Query::create()
		...
		->where("a.form_id = ? AND a.element_id = ?", array($element->form_id, $element->id))
		->groupBy('a.link_id');
	...
	if ($q->count() > 0) {
		...
		foreach ($filters as $filter) {
			$filter_js .= "filter_dropdown(" . $element->form_id . ", " . $element->id . ", " . $filter['link_id'] . ", this.value);";
		}
		$filter_js .= "'";
	}
	//OTB ADD
	...
	$element_markup = <<<EOT
		...
		<select class="form-control element select {$element->size}" id="element_{$element->id}" name="element_{$element->id}" {$filter_js} {$element->is_readonly} {$element->edit_style}>
			{$option_markup}
		</select>
		{$element_disabled}
		</div><span class="col-sm-10 col-sm-offset-2">{$guidelines} {$error_message}</span>
...
function mf_display_simple_name($element)
{
	...
	$attr_readonly = '';
	$guidelines = '';
	$el_class = array('simple_name form-group');
...
function mf_display_simple_name_wmiddle($element)
{
	...
	$element_markup = <<<EOT
		...
		<span class="simple_name_wmiddle_1">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="8" value="{$default_value_1}" {$element->edit_style}/>
			<label for="element_{$element->id}_1">{$mf_lang['name_first']}</label>
		</span>
		<span class="simple_name_wmiddle_2">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="8" value="{$default_value_2}" {$element->edit_style}/>
			<label for="element_{$element->id}_2">{$mf_lang['name_middle']}</label>
		</span>
		<span class="simple_name_wmiddle_3">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="14" value="{$default_value_3}" {$element->edit_style}/>
...
function mf_display_name($element)
{
	...
	$element_markup = <<<EOT
		...
		<span class="fullname_1">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="2" value="{$default_value_1}"/>
			<label for="element_{$element->id}_1">{$mf_lang['name_title']}</label>
		</span>
		<span class="fullname_2">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="8" value="{$default_value_2}" />
			<label for="element_{$element->id}_2">{$mf_lang['name_first']}</label>
		</span>
		<span class="fullname_3">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="14" value="{$default_value_3}" />
			<label for="element_{$element->id}_3">{$mf_lang['name_last']}</label>
		</span>
		<span class="fullname_4">
			<input id="element_{$element->id}_4" name="element_{$element->id}_4" {$attr_readonly} type="text" class="element form-control text" maxlength="255" size="3" value="{$default_value_4}"/>
...
function mf_display_name_wmiddle($element)
{
	...
	$attr_readonly = '';
	$guidelines = '';
	$el_class = array('fullname_wmiddle form-group');
	...
	$element_markup = <<<EOT
		<li id="li_{$element->id}" {$li_style} {$li_class} {$element->edit_style} class="form-group">
		<span class="description col-sm-2 control-label">{$element->title} {$span_required}</span>
		<span class="namewm_ext col-sm-2">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} type="text" class="element form-control text large" maxlength="255" value="{$default_value_1}" {$element->edit_style}/>
			<label for="element_{$element->id}_1">{$mf_lang['name_title']}</label>
		</span>
		<span class="namewm_first col-sm-2">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} type="text" class="element form-control text large" maxlength="255" value="{$default_value_2}" {$element->edit_style}/>
			<label for="element_{$element->id}_2">{$mf_lang['name_first']}</label>
		</span>
		<span class="namewm_middle col-sm-2">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} type="text" class="element form-control text large" maxlength="255" value="{$default_value_3}" {$element->edit_style}/>
			<label for="element_{$element->id}_3">{$mf_lang['name_middle']}</label>
		</span>
		<span class="namewm_last col-sm-2">
			<input id="element_{$element->id}_4" name="element_{$element->id}_4" {$attr_readonly} type="text" class="element form-control text large" maxlength="255" value="{$default_value_4}" {$element->edit_style}/>
			<label for="element_{$element->id}_4">{$mf_lang['name_last']}</label>
		</span>
		<span class="namewm_ext col-sm-2">
			<input id="element_{$element->id}_5" name="element_{$element->id}_5" {$attr_readonly} type="text" class="element form-control text large" maxlength="255" value="{$default_value_5}" {$element->edit_style}/>
...
function mf_display_time($element)
{
	...
	$attr_readonly = '';
	$guidelines = '';
	$el_class = array('time_field form-group');
	...
	if (!empty($element->time_showsecond)) {
		$seconds_markup = <<<EOT
		<span class="col-sm-2">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} class="element form-control text" size="2" type="text" maxlength="2" value="{$default_value_3}"/>
...
EOT;
	...
	if (empty($element->time_24hour)) {
		...
		$am_pm_markup = <<<EOT
		<span class="col-sm-2">
			<select class="element form-control select" style="width:5em" {$am_pm_disabled_attribute} id="element_{$element->id}_4" name="element_{$element->id}_4">
...
EOT;
	...
	$element_markup = <<<EOT
		...
		<span class="description col-sm-2 control-label">{$element->title} {$span_required}</span>
		<span class="col-sm-3">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element form-control text " size="2" type="text" maxlength="2" value="{$default_value_1}"/> :
			<label for="element_{$element->id}_1">{$mf_lang['time_hh']}</label>
		</span>
		<span class="col-sm-3">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} class="element form-control text " size="2" type="text" maxlength="2" value="{$default_value_2}" /> {$seconds_separator}
			<label for="element_{$element->id}_2">{$mf_lang['time_mm']}</label>
		</span>
		{$seconds_markup}
		{$am_pm_markup}
		<span class="col-sm-10 col-sm-offset-2">
		 {$guidelines} {$error_message}</span>
		</li>
EOT;
...
function mf_display_money($element)
{
	...
	if (!empty($el_class)) {
		...
		$li_class = 'class="' . rtrim($li_class) . ' form-group"';
	}
	...
	if ($element->constraint != 'yen') {
		...
		$element_markup = <<<EOT
		<li id="li_{$element->id}" {$li_style} {$li_class} {$price_data_tag} {$element->edit_style}>
		<span class="description col-sm-2 control-label">{$element->title} {$span_required}
		<span class="symbol">{$cur_symbol}</span>
		</span>
		<span class="col-sm-5">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element form-control text currency" size="10" value="{$default_value_1}" type="text"/> .
...
EOT;
		...
		$element_markup = <<<EOT
		<li id="li_{$element->id}" {$li_style} {$li_class} {$element->edit_style} {$price_data_tag}>
		<span class="description" for="element_{$element->id}">{$element->title} {$span_required}</span>
		<span class="symbol">{$cur_symbol}</span>
		<span>
			<input id="element_{$element->id}" name="element_{$element->id}" class="element form-control text currency" {$attr_readonly} size="10" value="{$default_value}" type="text" />
...
function mf_display_page_break($element)
{
	...
	if (empty($element->submit_primary_img)) {
		$element->submit_primary_img = '/form_builder/images/empty.gif';
	}

	if (empty($element->submit_secondary_img)) {
		$element->submit_secondary_img = '/form_builder/images/empty.gif';
	}
...
function mf_display_number($element)
{
	...
	//button for actual plot ratio or ground coverage
	$btn_actualgroundcoverage_or_actualplotratio = null;
	//OTB patch - Check if this field should be used for grand total for plinth area
	error_log("Grand total >>>> " . $element->grandtotalplintharea);
	//$btn_a_a_actualplotratio = null ;
	/////
	// error_log("this element footprint is >>>>".$element->footprint." And element id ".$element->id) ;
	//for elements that act as our footprint and also plinth area
	if (!empty($element->plotsize)) {
		...
		// $default_value = 0 ;
		//if the element is identified as plot size
		$hect_sqrmtr = "<label for='hect_sqrmt'> Unit of Measure</label><select id='hect_sqrmt'> <option value='none'> Select Unit </option><option value='sqr'> sqr meters </option> <option value='hect'> hectares </option></select> <div class='help'> You can enter plot size in hectares and select hectares as unit of measure. The system will convert the hectares to square meters </div>";
	} else if (!empty($element->projectcost)) {
		$myidentifier = "a_projectcost";
		//set default value for element 
	...
	//for plinth area elements
		...
		$attribute_disabled = "readonly";
		$myidentifier = "a_actualgroundcoverage";
		// error_log("this element is used as a_permittedgroundcoverage >>>>".$element->id) ;
		$warning = "               
			<div id='warning_groundcoverage_exceeded' class='alert alert-danger'> 
			 Warning! Your Actual Ground Coverge has exceeded permitted ground coverage for selected zone and permitted use. Please check your plinth area and plot size information before submitting or type in reasons below for exceeding.    
			</div> ";
...
function mf_get_country_list()
{
	...
	$country[37]['label'] = "Cayman Islands";
	$country[38]['label'] = "Central African Republic";
	$country[39]['label'] = "Chad";
	$country[40]['label'] = "Chile";
	$country[41]['label'] = "China";
	$country[42]['label'] = "Colombia";
	$country[43]['label'] = "Comoros";
	$country[44]['label'] = "Congo";
	$country[45]['label'] = "Costa Rica";
	$country[46]['label'] = "Côte d'Ivoire";
	$country[47]['label'] = "Croatia";
	$country[48]['label'] = "Cuba";
	$country[49]['label'] = "Cyprus";
	$country[50]['label'] = "Czech Republic";
	$country[51]['label'] = "Denmark";
	$country[52]['label'] = "Djibouti";
	$country[53]['label'] = "Dominica";
	$country[54]['label'] = "Dominican Republic";
	$country[55]['label'] = "East Timor";
	$country[56]['label'] = "Ecuador";
	$country[57]['label'] = "Egypt";
	$country[58]['label'] = "El Salvador";
	$country[59]['label'] = "Equatorial Guinea";
	$country[60]['label'] = "Eritrea";
	$country[61]['label'] = "Estonia";
	$country[62]['label'] = "Ethiopia";
	$country[63]['label'] = "Fiji";
	$country[64]['label'] = "Finland";
	$country[65]['label'] = "Gabon";
	$country[66]['label'] = "Gambia";
	$country[67]['label'] = "Georgia";
	$country[68]['label'] = "Ghana";
	$country[69]['label'] = "Greece";
	$country[70]['label'] = "Grenada";
	$country[71]['label'] = "Guatemala";
	$country[72]['label'] = "Guernsey";
	$country[73]['label'] = "Guinea";
	$country[74]['label'] = "Guinea-Bissau";
	$country[75]['label'] = "Guyana";
	$country[76]['label'] = "Haiti";
	$country[77]['label'] = "Honduras";
	$country[78]['label'] = "Hong Kong";
	$country[79]['label'] = "Hungary";
	$country[80]['label'] = "Iceland";
	$country[81]['label'] = "India";
	$country[82]['label'] = "Indonesia";
	$country[83]['label'] = "Iran";
	$country[84]['label'] = "Iraq";
	$country[85]['label'] = "Ireland";
	$country[86]['label'] = "Israel";
	$country[87]['label'] = "Italy";
	$country[88]['label'] = "Jamaica";
	$country[89]['label'] = "Japan";
	$country[90]['label'] = "Jordan";
	$country[91]['label'] = "Kazakhstan";
	$country[92]['label'] = "Kenya";
	$country[93]['label'] = "Kiribati";
	$country[94]['label'] = "North Korea";
	$country[95]['label'] = "South Korea";
	$country[96]['label'] = "Kuwait";
	$country[97]['label'] = "Kyrgyzstan";
	$country[98]['label'] = "Laos";
	$country[99]['label'] = "Latvia";
	$country[100]['label'] = "Lebanon";
	$country[101]['label'] = "Lesotho";
	$country[102]['label'] = "Liberia";
	$country[103]['label'] = "Libya";
	$country[104]['label'] = "Liechtenstein";
	$country[105]['label'] = "Lithuania";
	$country[106]['label'] = "Luxembourg";
	$country[107]['label'] = "Macedonia";
	$country[108]['label'] = "Madagascar";
	$country[109]['label'] = "Malawi";
	$country[110]['label'] = "Malaysia";
	$country[111]['label'] = "Maldives";
	$country[112]['label'] = "Mali";
	$country[113]['label'] = "Malta";
	$country[114]['label'] = "Marshall Islands";
	$country[115]['label'] = "Mauritania";
	$country[116]['label'] = "Mauritius";
	$country[117]['label'] = "Mexico";
	$country[118]['label'] = "Micronesia";
	$country[119]['label'] = "Moldova";
	$country[120]['label'] = "Monaco";
	$country[121]['label'] = "Mongolia";
	$country[122]['label'] = "Montenegro";
	$country[123]['label'] = "Morocco";
	$country[124]['label'] = "Mozambique";
	$country[125]['label'] = "Myanmar";
	$country[126]['label'] = "Namibia";
	$country[127]['label'] = "Nauru";
	$country[128]['label'] = "Nepal";
	$country[129]['label'] = "New Zealand";
	$country[130]['label'] = "Nicaragua";
	$country[131]['label'] = "Niger";
	$country[132]['label'] = "Nigeria";
	$country[133]['label'] = "Norway";
	$country[134]['label'] = "Oman";
	$country[135]['label'] = "Pakistan";
	...
	$country[37]['value'] = "Cayman Islands";
	$country[38]['value'] = "Central African Republic";
	$country[39]['value'] = "Chad";
	$country[40]['value'] = "Chile";
	$country[41]['value'] = "China";
	$country[42]['value'] = "Colombia";
	$country[43]['value'] = "Comoros";
	$country[44]['value'] = "Congo";
	$country[45]['value'] = "Costa Rica";
	$country[46]['value'] = "Côte d'Ivoire";
	$country[47]['value'] = "Croatia";
	$country[48]['value'] = "Cuba";
	$country[49]['value'] = "Cyprus";
	$country[50]['value'] = "Czech Republic";
	$country[51]['value'] = "Denmark";
	$country[52]['value'] = "Djibouti";
	$country[53]['value'] = "Dominica";
	$country[54]['value'] = "Dominican Republic";
	$country[55]['value'] = "East Timor";
	$country[56]['value'] = "Ecuador";
	$country[57]['value'] = "Egypt";
	$country[58]['value'] = "El Salvador";
	$country[59]['value'] = "Equatorial Guinea";
	$country[60]['value'] = "Eritrea";
	$country[61]['value'] = "Estonia";
	$country[62]['value'] = "Ethiopia";
	$country[63]['value'] = "Fiji";
	$country[64]['value'] = "Finland";
	$country[65]['value'] = "Gabon";
	$country[66]['value'] = "Gambia";
	$country[67]['value'] = "Georgia";
	$country[68]['value'] = "Ghana";
	$country[69]['value'] = "Greece";
	$country[70]['value'] = "Grenada";
	$country[71]['value'] = "Guatemala";
	$country[72]['value'] = "Guernsey";
	$country[73]['value'] = "Guinea";
	$country[74]['value'] = "Guinea-Bissau";
	$country[75]['value'] = "Guyana";
	$country[76]['value'] = "Haiti";
	$country[77]['value'] = "Honduras";
	$country[78]['value'] = "Hong Kong";
	$country[79]['value'] = "Hungary";
	$country[80]['value'] = "Iceland";
	$country[81]['value'] = "India";
	$country[82]['value'] = "Indonesia";
	$country[83]['value'] = "Iran";
	$country[84]['value'] = "Iraq";
	$country[85]['value'] = "Ireland";
	$country[86]['value'] = "Israel";
	$country[87]['value'] = "Italy";
	$country[88]['value'] = "Jamaica";
	$country[89]['value'] = "Japan";
	$country[90]['value'] = "Jordan";
	$country[91]['value'] = "Kazakhstan";
	$country[92]['value'] = "Kenya";
	$country[93]['value'] = "Kiribati";
	$country[94]['value'] = "North Korea";
	$country[95]['value'] = "South Korea";
	$country[96]['value'] = "Kuwait";
	$country[97]['value'] = "Kyrgyzstan";
	$country[98]['value'] = "Laos";
	$country[99]['value'] = "Latvia";
	$country[100]['value'] = "Lebanon";
	$country[101]['value'] = "Lesotho";
	$country[102]['value'] = "Liberia";
	$country[103]['value'] = "Libya";
	$country[104]['value'] = "Liechtenstein";
	$country[105]['value'] = "Lithuania";
	$country[106]['value'] = "Luxembourg";
	$country[107]['value'] = "Macedonia";
	$country[108]['value'] = "Madagascar";
	$country[109]['value'] = "Malawi";
	$country[110]['value'] = "Malaysia";
	$country[111]['value'] = "Maldives";
	$country[112]['value'] = "Mali";
	$country[113]['value'] = "Malta";
	$country[114]['value'] = "Marshall Islands";
	$country[115]['value'] = "Mauritania";
	$country[116]['value'] = "Mauritius";
	$country[117]['value'] = "Mexico";
	$country[118]['value'] = "Micronesia";
	$country[119]['value'] = "Moldova";
	$country[120]['value'] = "Monaco";
	$country[121]['value'] = "Mongolia";
	$country[122]['value'] = "Montenegro";
	$country[123]['value'] = "Morocco";
	$country[124]['value'] = "Mozambique";
	$country[125]['value'] = "Myanmar";
	$country[126]['value'] = "Namibia";
	$country[127]['value'] = "Nauru";
	$country[128]['value'] = "Nepal";
	$country[129]['value'] = "New Zealand";
	$country[130]['value'] = "Nicaragua";
	$country[131]['value'] = "Niger";
	$country[132]['value'] = "Nigeria";
	$country[133]['value'] = "Norway";
	$country[134]['value'] = "Oman";
	$country[135]['value'] = "Pakistan";
	...
	$element_markup = <<<EOT
		...
		<div class="col-sm-10" >
			...
			<span id="li_{$element->id}_span_2" {$address_line2_style}>
				<input id="element_{$element->id}_2" name="element_{$element->id}_2" class="element form-control text large" {$attr_readonly} value="{$default_value_2}" type="text" />
				<label for="element_{$element->id}_2">{$mf_lang['address_street2']}</label>
			</span>

			<span id="li_{$element->id}_span_3" class="left state_list">
				<input id="element_{$element->id}_3" name="element_{$element->id}_3" class="element form-control text large" {$attr_readonly} value="{$default_value_3}" type="text" />
			...
			<span id="li_{$element->id}_span_5" class="left">
				<input id="element_{$element->id}_5" name="element_{$element->id}_5" class="element form-control text large" {$attr_readonly} maxlength="15" value="{$default_value_5}" type="text" />
				<label for="element_{$element->id}_5">{$mf_lang['address_zip']}</label>
			</span>

			<span id="li_{$element->id}_span_6" class="right">
				<select class="element form-control select large" id="element_{$element->id}_6" name="element_{$element->id}_6" {$country_disabled_attr}>
				{$country_markup}
				</select>
	...
	if (!empty($element->matrix_children)) {
		foreach ($element->matrix_children as $matrix_item) {
			...
			foreach ($children_option_id as $option_id) {
				...
				$td_markup .= "<td><label style=\"display: none\" for=\"element_{$matrix_item['id']}_{$option_id}\">{$option_titles[$i]}</label><input id=\"element_{$matrix_item['id']}_{$option_id}\" name=\"element_{$matrix_item['id']}{$option_id_var}\" type=\"{$input_type}\" value=\"{$option_id}\" {$checked_markup} /></td>\n";
				$i++;
			}
...
//Plinth Area - OTB patch - Show plinth area to the user
function mf_display_plinth_area($element)
{
	...
	//check for guidelines
	if (!empty($element->guidelines)) {
		$guidelines = "<p class=\"guidelines col-sm-12\" id=\"guide_{$element->id}\"><small>{$element->guidelines}</small></p>";
	}
	...
	if (empty($default_value_1) && empty($default_value_2)) {
		$default_value_1 = 0;
		$default_value_2 = 0;
	}
	...
	$element_markup = <<<EOT
		...
		<div class="plinth_area_1  col-sm-4" >
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" type="number" class="form-control {$identifier_new}" maxlength="255" size="8" value="{$default_value_1}" {$element->is_readonly} />
			<label for="element_{$element->id}_1">{$plinth_area_new}</label>
		</div>
		<div class="plinth_area_2 col-sm-4" >
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" type="number" class="form-control {$identifier_existing}" maxlength="255" size="14" value="{$default_value_2}" {$element->is_readonly}/>
			<label for="element_{$element->id}_2">{$plinth_area_first}</label>
...
//Plinth Area - OTB patch - Show total plinth area to the user
function mf_display_total_plinth_area($element)
{
	...
	$span_required = '';
	$guidelines = '';
	$el_class = array('total_plinth_area');
	...
	$element_markup = <<<EOT
		...
		<div class="total_plinth_area_2 col-sm-4">
			...
			<label for="element_{$element->id}_2">{$total_plinth_area_first}</label>
                        <a href="javascript:;" class="btn btn-warning" id="btn_exisitng_plinth"> Update Total Existing</a>
		...
		<script>
		$(function(){
			$('#btn_new_plinth').click(function(e){
				var total_new=0;
				$('.plintharea_new').each(function(i,el){
					total_new+=parseFloat($(this).val());
				});
				$("#element_{$element->id}_1").val(total_new);
			});
			$('#btn_exisitng_plinth').click(function(e){
				var total_existing=0;
				$('.plintharea_existing').each(function(i,el){
					total_existing+=parseFloat($(this).val());
				});
				$("#element_{$element->id}_2").val(total_existing);
			});
		});
		</script>
EOT;
...
function mf_display_form($dbh, $form_id, $form_params = array())
{
	...
	if ($row === false) {
		...
		return "<ul><li>No form has been configured.</li></ul>";
	} else {
	...
	if ($form->payment_enable_merchant && $form->payment_price_type == 'variable') {
		$query = "select
							element_id,
							option_id,
							`price`
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		if ($trans_text) {
			$options_lookup[$element_id][$option_id]['option'] = $trans_text;
		}
	...
	while ($row = mf_do_fetch_result($sth)) {
		...
		//OTB if plinth area set javascript
		if ($row['element_type'] == 'total_plinth_area') {
			$total_plinth_area = true;
		}
	...
	foreach ($element as $element_data) {
		...
		if ($edit_id and $element_data->is_required and !$populated_elem_values) {
			...
			if ($edit_fields) {
				$element_data->is_readonly = in_array($element_data->id, $edit_fields) ? "" : ($selectable ? "disabled" : "readonly");
				$element_data->edit_style = in_array($element_data->id, $edit_fields) ? "style=\"border: 2px solid #dadada;border-radius: 7px;outline: none;border-color: green;box-shadow: 0 0 10px #9ecaed;\"" : "";
				if ($selectable) {
					foreach ($element_keys as $element_key) {
						$edit_fields_markup .= "\$('#form_{$form_id}').submit(function() {
													\$('#{$element_key}').removeAttr('disabled');
													});";
					}
				}
			}
	...
	if (!empty($custom_error)) {
		if (is_array($custom_error)) {
			foreach ($custom_error as $error => $desc) {
				$form->error_message .= <<<EOT
				<li id="error_message">
						<h5 id="error_message_title" class="text-danger">{$desc}</h5>
				</li>
	EOT;
			}
		} else {
			$form->error_message = <<<EOT
				<li id="error_message">
						<h3 id="error_message_title" class="text-danger">{$custom_error}</h3>
				</li>
	EOT;
		}
	...
	if (!empty($form->theme_id) && empty($edit_id)) {
		...
		if (empty($form->review)) {
			if ($row['form_button_type'] == 'text') {
				$submit_button_markup = '<input id="submit_form" class="button_text btn-form" type="submit" name="submit_form" value="' . $row['form_button_text btn-form'] . '" />';
			} else {
			...
			if ($row['form_button_type'] == 'text') {
				$submit_button_markup = '<input id="submit_form" class="button_text btn-form" type="submit" name="submit_form" value="' . $mf_lang['continue_button'] . '" />';
			} else {
				$submit_button_markup = '<input class="submit_img_primary" type="image" alt="Submit" id="submit_form" name="submit_form" src="' . $row['form_button_image'] . '" />';
			}
		}
		...
		if (empty($form->review)) {
			$submit_button_markup = '<input id="submit_form" class="button_text btn-form" type="submit" name="submit_form" value="' . $mf_lang['submit_button'] . '" />';
		} else {
			$submit_button_markup = '<input id="submit_form" class="button_text btn-form  btn-form" type="submit" name="submit_form" value="' . $mf_lang['continue_button'] . '" />';
		}
	}
...
EOT;
	...
	if ($has_calendar) {
		...
		switch ($form->language) {
			case 'dutch':
				$datepick_lang = 'js/datepick/jquery.datepick-nl.js';
				break;
			case 'french':
				$datepick_lang = 'js/datepick/jquery.datepick-fr.js';
				break;
			case 'german':
				$datepick_lang = 'js/datepick/jquery.datepick-de.js';
				break;
			case 'italian':
				$datepick_lang = 'js/datepick/jquery.datepick-it.js';
				break;
			case 'portuguese':
				$datepick_lang = 'js/datepick/jquery.datepick-pt.js';
				break;
			case 'spanish':
				$datepick_lang = 'js/datepick/jquery.datepick-es.js';
				break;
			case 'swedish':
				$datepick_lang = 'js/datepick/jquery.datepick-sv.js';
				break;
			case 'japanese':
				$datepick_lang = 'js/datepick/jquery.datepick-ja.js';
				break;
			case 'estonian':
				$datepick_lang = 'js/datepick/jquery.datepick-et.js';
				break;
			case 'russian':
				$datepick_lang = 'js/datepick/jquery.datepick-ru.js';
				break;
			case 'hungarian':
				$datepick_lang = 'js/datepick/jquery.datepick-hu.js';
				break;
			case 'chinese':
				$datepick_lang = 'js/datepick/jquery.datepick-zh-TW.js';
				break;
			case 'chinese_simplified':
				$datepick_lang = 'js/datepick/jquery.datepick-zh-CN.js';
				break;
			case 'bulgarian':
				$datepick_lang = 'js/datepick/jquery.datepick.js-bg';
				break;
			case 'danish':
				$datepick_lang = 'js/datepick/jquery.datepick-da.js';
				break;
			case 'finnish':
				$datepick_lang = 'js/datepick/jquery.datepick-fi.js';
				break;
			case 'polish':
				$datepick_lang = 'js/datepick/jquery.datepick-pl.js';
				break;
			case 'greek':
				$datepick_lang = 'js/datepick/jquery.datepick-el.js';
				break;
			case 'norwegian':
				$datepick_lang = 'js/datepick/jquery.datepick-no.js';
				break;
			case 'romanian':
				$datepick_lang = 'js/datepick/jquery.datepick-ro.js';
				break;
			case 'slovak':
				$datepick_lang = 'js/datepick/jquery.datepick-sk.js';
				break;
			case 'indonesia':
				$datepick_lang = 'js/datepick/jquery.datepick-id.js';
				break;
			default:
...
EOT;
			...
			if (!empty($form->review) && $form->payment_enable_merchant && $form->payment_onsubmission) {
			...
			} else if (!empty($form->review) || ($form->payment_enable_merchant && $form->payment_onsubmission)) {
			...
			if (empty($page_breaks_data[$page_number]['use_image'])) { //if using text buttons as submit
				...
				if ($page_number > 1) {
					...
					if (!empty($form->resume_enable)) {
						$button_secondary_markup .= ' <input class="btn btn-success" type="submit" onClick="document.getElementById(\'save_as_draft\').value = 1;" value="' . $mf_lang['resume_checkbox_title'] . '" /> <input type="hidden" name="save_as_draft" id="save_as_draft" value="0">';
					}
				}
...
EOT;
				...
				if ($page_number > 1) {
					$button_secondary_markup = '<input class="btn btn-success submit_img_secondary" type="image" alt="Previous" id="submit_secondary" name="submit_secondary" src="' . $page_breaks_data[$page_number]['secondary_img'] . '" />';
					if (!empty($form->resume_enable)) {
						$button_secondary_markup .= ' <input class="btn btn-success submit_img_secondary" type="image" alt="Draft" onClick="document.getElementById(\'save_as_draft\').value = 1;" src="' . $page_breaks_data[$page_number]['secondary_img'] . '" /><input type="hidden" name="save_as_draft" id="save_as_draft" value="0">';
					}
				}
...
EOT;
	...
	if ($has_advance_uploader) {
		...
		if (!empty($machform_path)) {
			$mf_path_script = <<<EOT
<script type="text/javascript">
var __machform_path = '/form_builder/';
</script>
EOT;
		}
...
EOT;
	...
	if ($integration_method == 'iframe') {
		$auto_height_js = <<<EOT
<script type="text/javascript" src="/form_builder/js/jquery.ba-postmessage.min.js"></script>
<script type="text/javascript">
    $(function(){
    	$.postMessage({mf_iframe_height: $('body').outerHeight(true)}, '*', parent );
    });
...
EOT;
   ...
   EOT;
	...
	if ($form->payment_enable_merchant && $form->payment_onsubmission && !empty($form->payment_show_total)) {
		if (strlen($form->payment_currency)) {
			$currency_symbol = $form->payment_currency;
		} else {
			$currency_symbol = '&#36;';
		}
...
EOT;
	...
	if ($integration_method == 'php') {
		...
		if (!empty($edit_id)) {
			$view_css_markup = '<link rel="stylesheet" type="text/css" href="/form_builder/css/edit_entry.css" media="all" />';
		} else {
			$view_css_markup = "<link rel=\"stylesheet\" type=\"text/css\" href=\"/form_builder/{$css_dir}view.css\" media=\"all\" />";
		}
...
<script type="text/javascript" src="/form_builder/js/jquery-ui/ui/jquery.effects.core.js"></script>
<script type="text/javascript" src="/form_builder/view.js"></script>
{$recaptcha2_header}
{$advance_uploader_js}
{$calendar_init}
{$signature_pad_init}
...
function mf_display_raw_form($dbh, $form_id)
{
	...
	$query = "select
						...
						element_zone,
						element_projectcost,
						element_groundcoveragereason,
						element_grandtotalplintharea,
						element_plotratioreason,
...
EOT;
		...
		if (empty($form->submit_primary_img)) {
			$form->submit_primary_img = '/form_builder/images/empty.gif';
		}

		if (empty($form->submit_secondary_img)) {
			$form->submit_secondary_img = '/form_builder/images/empty.gif';
		}
...
function mf_display_form_review($dbh, $form_id, $record_id, $from_page_num, $form_params = array())
{
	...
	$entry_data = '<table id="machform_review_table" class="table table-hover table-special table-striped" width="100%" border="0" cellspacing="0" cellpadding="0"><tbody>' . "\n";
...
EOT;
	...
	if ($integration_method == 'php') {
		...
		if ($has_signature_field) {
			$signature_pad_init = '<!--[if lt IE 9]><script src="/form_builder/js/signaturepad/flashcanvas.js"></script><![endif]-->' . "\n" .
...
function mf_display_form_payment($dbh, $form_id, $record_id, $form_params = array())
{
	...
	error_log('-----integration_method-----' . $integration_method);
	...
	if (!empty($form_params['invoice'])) {
		$_SESSION['mf_invoice'] = $form_params['invoice'];
...
EOT;
	...
	if (!empty($payment_ask_billing)) {
		$billing_address_markup = <<<EOT
				<li id="li_billing_address" class="address form-group">
					<label class="description">Billing Address <span class="required">*</span></label>
					<div>
						<span id="li_billing_span_1">
							<input id="billing_street" class="element form-control text large" value="" type="text" />
							<label for="billing_street">{$mf_lang['address_street']}</label>
						</span>

						<span id="li_billing_span_2" class="left state_list">
							<input id="billing_city" class="element form-control text large" value="" type="text" />
							<label for="billing_city">{$mf_lang['address_city']}</label>
						</span>

						<span id="li_billing_span_3" class="right state_list">
							<input id="billing_state" class="element form-control text large" value="" type="text" />
							<label for="billing_state">{$mf_lang['address_state']}</label>
						</span>

						<span id="li_billing_span_4" class="left">
							<input id="billing_zipcode" class="element form-control text large" maxlength="15" value="{$default_value_5}" type="text" />
...
EOT;
		...
		$shipping_address_markup = <<<EOT
				<li id="li_shipping_address" class="address form-group">
					...
					<div class="shipping_address_detail" style="{$shipping_display}">
						<span id="li_shipping_span_1">
							<input id="shipping_street" class="element form-control text large" value="" type="text" />
							<label for="shipping_street">{$mf_lang['address_street']}</label>
						</span>

						<span id="li_shipping_span_2" class="left state_list">
							<input id="shipping_city" class="element form-control text large" value="" type="text" />
							<label for="shipping_city">{$mf_lang['address_city']}</label>
						</span>

						<span id="li_shipping_span_3" class="right state_list">
							<input id="shipping_state" class="element form-control text large" value="" type="text" />
							<label for="shipping_state">{$mf_lang['address_state']}</label>
						</span>

						<span id="li_shipping_span_4" class="left">
							<input id="shipping_zipcode" class="element form-control text large" maxlength="15" value="{$default_value_5}" type="text" />
							<label for="shipping_zipcode">{$mf_lang['address_zip']}</label>
						</span>

						<span id="li_shipping_span_5" class="right">
							<select class="element form-control select large" id="shipping_country">
								{$country_markup}
							</select>
...
<script type="text/javascript" src="/form_builder/js/payment_stripe.js"></script>
EOT;
		...
		$merchant_js = <<<EOT
<script type="text/javascript" src="/form_builder/js/jquery.payment.js"></script>
<script type="text/javascript" src="/form_builder/js/payment_authorizenet.js"></script>
EOT;
	} else if ($payment_merchant_type == 'paypal_rest') {

		$merchant_js = <<<EOT
<script type="text/javascript" src="/form_builder/js/jquery.payment.js"></script>
<script type="text/javascript" src="/form_builder/js/payment_paypal_rest.js"></script>
EOT;
...
<script type="text/javascript" src="/form_builder/js/payment_braintree1.1.js"></script>
EOT;
	} else if ($payment_merchant_type == 'pesaflow_standard' || $payment_merchant_type == 'pesaflow_checkout') {
		$integration_method = "iframe";
	}
...
<div id="main_body" class="integrated no_guidelines" data-machformpath="/form_builder/">
	<div id="form_container">
		<form id="form_{$form_id}" class="appnitro" method="post" action="javascript:" data-highlightcolor="{$field_highlight_color}">
			...
			<ul class="payment_detail_form">
				...
				<li id="li_credit_card" class="credit_card">
					...
					<div>
						<span id="li_cc_span_1" class="left">
							<input id="cc_first_name" class="element form-control text large" value="" type="text" />
							<label for="cc_first_name">First Name</label>
						</span>

						<span id="li_cc_span_2" class="right">
							<input id="cc_last_name" class="element form-control text large" value="" type="text" />
							<label for="cc_last_name">Last Name</label>
						</span>

						<span id="li_cc_span_3" class="left">
							<input id="cc_number" class="element form-control text large" value="" type="text" />
							<label for="cc_number">Credit Card Number</label>
						</span>

						<span id="li_cc_span_4" class="right">
							<input id="cc_cvv" class="element form-control text large" value="" type="text" />
							<label for="cc_cvv">CVV</label>
						</span>

						<span id="li_cc_span_5" style="text-align: right">
							<img id="cc_secure_icon" src="/form_builder/images/icons/lock.png" alt="Secure" title="Secure" />
							<label for="cc_expiry_month" style="display: inline">Expiration: </label>
							<select class="element form-control select" id="cc_expiry_month">
							...
							<select class="element form-control select" id="cc_expiry_year">
								{$year_dropdown_markup}
							</select>
						</span>
				...
				<li id="li_buttons" class="buttons">
				    ...
				    {$button_markup}
				    <img id="mf_payment_loader_img" style="display: none" src="/form_builder/images/loader_small_grey.gif" />
				</li>
			</ul>
		</form>
...
EOT;
		...
		if ($payment_merchant_type == 'pesaflow_standard' || $payment_merchant_type == 'pesaflow_checkout') {
			error_log('------------pesaflow_standard---------');
			$pesaflow = new PesaflowGateway();
			...
			if ($_SESSION['mf_invoice']) {
				...
				if ($payment_merchant_type == 'pesaflow_standard') {
					if ($form_params['backend']) {
						$form_markup = $pesaflow->checkout_standard($application->getId(), true);
					} else {
						$form_markup = $pesaflow->checkout_standard($application->getId(), false);
					}
				} elseif ($payment_merchant_type == 'pesaflow_checkout') {
					$form_markup = $pesaflow->checkout_cart($application->getId(), false);
				}
			} else {
				if ($_SESSION['profile_id']) {
					$form_markup = $pesaflow->checkout_profile($_SESSION['profile_id'], $total_payment_amount, false);
				} else {
					$q = Doctrine_Query::create()
						->from("FormEntry a")
						->where("a.form_id = ? AND a.entry_id = ?", array($form_id, $record_id))
						->orWhere("a.form_id = ? AND a.entry_id = ?", array($form_id, $_SESSION['mf_payment_record_id'][$form_id]));
		...
		} elseif ($payment_merchant_type == 'cash' || $payment_merchant_type == 'check') {
			$cash = new CashGateway();
			$form_markup = $cash->checkout($_SESSION['mf_invoice'], []);
		} elseif ($payment_merchant_type == 'zizi') {
			$cash = new ZiziGateway();
			$form_markup = $cash->checkout($_SESSION['mf_invoice']);
		} elseif ($payment_merchant_type == 'malipo') {
			error_log("------ Debug Malipo Payment Gateway ------");

			$payment = new MalipoGateway();
			$form_markup = $payment->checkout_malipo_payment_links($_SESSION['mf_invoice']);
		} else {
