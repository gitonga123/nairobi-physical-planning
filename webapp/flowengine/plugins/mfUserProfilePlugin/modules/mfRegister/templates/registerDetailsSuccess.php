		...
		if ($submit_result['status'] === true) {
			if (true) {
				...
				if (is_numeric($target_page_id)) {
					header("Location: http{$ssl_suffix}://" . "nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . "?formid={$_GET['formid']}&id={$input_array['form_id']}&mf_page={$target_page_id}");
					exit;
					...
					if (empty($submit_result['form_redirect'])) {
						header("Location: http{$ssl_suffix}://" . "nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . "?formid={$_GET['formid']}&id={$input_array['form_id']}&entryid={$submit_result['membership_entry_id']}&done=1");
						exit;
					} else {
						echo "<script type=\"text/javascript\">top.location.replace('{$submit_result['form_redirect']}')</script>";
						exit;
				...
				if (!empty($submit_result['next_page_number'])) {
					...
					header("Location: http{$ssl_suffix}://" . "nairobipay.go.ke" . str_replace(['index.php', 'backend.php'], 'plan', $_SERVER['PHP_SELF']) . "?formid={$_GET['formid']}&id={$input_array['form_id']}&mf_page={$submit_result['next_page_number']}");
					exit;
				} else {
