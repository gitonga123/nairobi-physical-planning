...
class sfApplyApplyForm extends sfGuardUserProfileForm
{
  ...
  public function configure()
  {
    ...
    $this->setWidget('registeras', new sfWidgetFormDoctrineChoice(
      array(
		'model' => 'SfGuardUserCategories',
		'add_empty' => false,
	  )
    ));
	...
	$this->setWidget('profile_pic',new sfWidgetFormInputFileEditable(
		array(
			'file_src' => sfConfig::get('sf_web_dir').'/assets_frontend/images/'.$this->getObject()->getProfilePic(),
			'is_image' => true,
			'with_delete' => true,
			'edit_mode' => !$this->isNew() && $this->getObject()->getProfilePic(),
		)
	));	
	...
	// $this->setValidator('security_code', new sfValidatorString(array(
   //   'required' => true
  //  )));
    ...
    $this->setValidator('registeras', new sfValidatorDoctrineChoice(array(
		'model' => 'SfGuardUserCategories',
      'required' => true
    )));
    ...
    $this->setValidator('mobile', new sfValidatorAnd(array(
      new sfValidatorString(array(
        'required' => true,
        'trim' => true,
        'min_length' => 10,
	...
	$this->setValidator('profile_pic', new sfValidatorFile(
		array(
			'mime_types' => 'web_images',
			'path' => sfConfig::get('sf_web_dir').'/assets_frontend/images',
			  'required' => false,
		)
	));
	$this->setValidator('profile_pic_delete', new sfValidatorBoolean());
      ...
      new sfValidatorAnd(
        array(
          ...
          new sfValidatorDoctrineUnique(array(
              ...
              'column' => 'email'
            ), array('invalid' => 'This email has already been used. Try another')
          )
        )
      )
    );
