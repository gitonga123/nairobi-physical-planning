...
$reconciliation_log.="/**** STARTED PESAFLOW RECONCILE DAILY ".date("Y-m-d H:i:s")." ***/\n <br>";
error_log("Cronlog: Ended Pesaflow Service Reconciliation Cronjob ".date("Y-m-d H:i:s"));

$q = Doctrine_Query::create()
    ->from("ApFormPayments a")
    ->where("a.payment_date LIKE ?", date("Y-m-d", strtotime('-0 day'))."%")
...
$reconciliation_log.="/**** ENDED PESAFLOW RECONCILE DAILY ".date("Y-m-d H:i:s")." ***/\n <br>";
...
try 
{
    ...
    $notifier->sendemail($organisation_email, "thomasjgx@gmail.com","Reconciliation Log (Daily) - ".$organisation_name." ".date("Y-m-d H:i:s"),$reconciliation_log);
}
catch(Exception $ex)
{
