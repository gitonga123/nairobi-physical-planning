...
$(function(){
	...
	$(".middle_form_bar > h3").click(function(){
		...
		$("#" + selected_form_li_id + " .form_option").promise().done(function() {
			$(this).parent().parent().toggleClass('form_selected');
		});
	...
	$(".mf_link_disable").click(function(){
		var selected_form_li_id = $(this).parent().parent().parent().attr('id');
		...
		if(current_action == 'disable'){
			...
			$.ajax({
				   ...
				   error: function(xhr,text_status,e){
					  ...
					  $("#" + selected_form_li_id + " .mf_link_disable").text(current_action);
				   ...
				   success: function(response_data){
					   ...
					   if(response_data.status == 'ok'){
						   if(response_data.action == 'disable'){
							   ...
							   $("#liform_" + response_data.form_id + " .mf_link_disable").html('<span class="icon-play"></span>Enable');
							   $("#liform_" + response_data.form_id + " .mf_link_disable img").remove();
						   }else{
							   $("#liform_" + response_data.form_id).removeClass('form_inactive');
							   $("#liform_" + response_data.form_id + " .mf_link_disable").html('<span class="icon-pause"></span>Disable');
							   $("#liform_" + response_data.form_id + " .mf_link_disable img").remove();
						   }
					   }else{
		...
		buttons: [{
				...
				click: function() {
					...
					if($("#dialog-disabled-message-input").val() == ""){
						...
						$("#dialog-disabled-message-btn-save").after("<div class='small_loader_box'><img src='/form_builder/images/loader_small_grey.gif' /></div>");
		...
		buttons: [{
				...
				click: function() {
					...
					if($("#dialog-enter-tagname-input").val() == ""){
						...
						$("#liform_" + form_id + " ul.form_tag_list").append("<li class=\"processing\"><img src='/form_builder/images/loader_small_grey.gif' /></li>");
	...
	$('#mf_form_list').delegate('a.removetag', 'click', function(e) {
		...
		if($(this).find('img').attr("src") != "/form_builder/images/loader_green_16.png"){
			$(this).find('img').attr("src","/form_builder/images/loader_green_16.png");
			...
			$.ajax({
				   ...
				   dataType: "json",
				   error: function(xhr,text_status,e){
					    parent_list.find('img').attr("src","/form_builder/images/icons/53.png");
	...
	$(".mf_link_duplicate").click(function(){
		var selected_form_li_id = $(this).parent().parent().parent().attr('id');
		...
		//display the loader image
		$(this).parent().css("position","relative");
		$(this).before('<img src="/form_builder/images/loader_small_grey.gif" style="position: absolute;margin-left: -28px" />');
	...
	$("#dialog-confirm-form-delete").dialog({
		...
		buttons: [{
				...
				click: function() {
					...
					$("#btn-form-delete-ok").after("<div class='small_loader_box'><img src='/form_builder/images/loader_small_grey.gif' /></div>");
	...
	$("#dialog-change-theme").dialog({
		...
		buttons: [{
				...
				click: function() {
					...
					$("#btn-change-theme-ok").after("<div class='small_loader_box'><img src='/form_builder/images/loader_small_grey.gif' /></div>");
