$(function(){
	...
	$('#ps_onsubmission').bind('change', function() {
		if($(this).prop("checked") == true){
			$("#ps_main_list").data('payment_properties').onsubmission = 1;
		}else{
			$("#ps_main_list").data('payment_properties').onsubmission = 0;
		}
	});
	$('#ps_merchant_identifier').bind('change', function() {
			$("#ps_main_list").data('payment_properties').merchant_identifier = $.trim($(this).val());
	});
	...
	$('#ps_select_merchant').bind('change', function() {
		...
		$(".merchant_options,.stripe_option,.paypal_option,.authorizenet_option,.braintree_option,.pesaflow_standard_option,.pesaflow_cart_option,.paypal_rest_option").hide();
		$("#ps_currency_paypal_div,#ps_currency_stripe_div,#ps_currency_authorizenet_div,#ps_currency_braintree_div, #ps_currency_pesaflow_standard_div, #ps_currency_pesaflow_cart_div,#ps_currency_paypal_rest_div,#ps_currency_check_div,#merchant_currency").hide();
		
		//reset the currency to USD
		$("#ps_main_list").data('payment_properties').currency = 'USD';
		$('#ps_currency_paypal,#ps_currency_stripe,#ps_currency_authorizenet,#ps_currency_braintree, #ps_currency_pesaflow_standard_div, #ps_currency_pesaflow_cart_div, #ps_currency_paypal_rest,#ps_currency_check').val('USD');
		...
		if(merchant_type == 'paypal_standard'){
		...
		}else if(merchant_type == 'pesaflow_standard'){
			$("#ps_pesaflow_standard_options").show();
			$(".pesaflow_standard_option").css("display","block").show();

			$("#ps_currency_pesaflow_standard_div").show();
		}else if(merchant_type == 'pesaflow_cart'){
			$("#ps_pesaflow_cart_options").show();
			$(".pesaflow_cart_option").css("display","block").show();

			$("#ps_currency_pesaflow_cart_div").show();
			...
			$.ajax({
				...
				$('#currency_check').children().remove();
				$('#currency_check').append('<option value="'+rep.symbol+'" selected="selected">'+rep.symbol+' - '+rep.currency+'</option>');
				$("#ps_main_list").data('payment_properties').currency = rep.symbol;
    ...
    $("#ps_pesaflow_api_id").bind("keyup mouseout change", function () {
        $("#ps_main_list").data("payment_properties").cellulant_checkout_url = $(this).val()
    });

    $("#ps_pesaflow_key").bind("keyup mouseout change", function () {
        $("#ps_main_list").data("payment_properties").cellulant_merchant_username = $(this).val()
    });

    $("#ps_pesaflow_secret").bind("keyup mouseout change", function () {
        $("#ps_main_list").data("payment_properties").cellulant_merchant_password = $(this).val()
    });

    $("#ps_pesaflow_service_id").bind("keyup mouseout change", function () {
	...
	$('#ps_currency_paypal,#ps_currency_stripe,#ps_currency_authorizenet,#ps_currency_braintree,#ps_currency_pesaflow_standard,#ps_currency_pesaflow_cart,#ps_currency_paypal_rest,#ps_currency_check,#currency_check').bind('change', function() {
	...
	$('#ps_price_name,#ps_stripe_charge_desc').bind('keyup mouseout change', function() {
		...
		$(this).children("option:selected").each(function(i,e){
			$("#ps_price_amount").val($(e).data('amount'));
			if(Number($(e).data('fixed'))){
				$("#ps_price_amount").attr('readonly','readonly');
			}
		});
	});
