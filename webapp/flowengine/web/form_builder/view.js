//OTB add javascript for element here
$(function(){
...
function remove_attachment(filename,form_id,element_id,holder_id,is_db_live,key_id,saved_filename=null){
	
	var machform_path = '/form_builder/';
		   ...
		   success: function(response_data){
			   if(response_data.status == 'ok'){
				   ...
				   if(is_support_html5_uploader()){
				   	   try{
				   	   		$("#"+response_data.holder_id).uploadifive('cancel',$("#" + response_data.holder_id).data('file'));
							console.log('--------uploadfive---');
				   	   }catch(e){
						   $("#" + response_data.holder_id).fadeOut("slow",function(){$(this).remove();});
							console.log('--------no--uploadfive---');
					   }
				   }else{
				       $("#" + response_data.holder_id).fadeOut("slow",function(){$(this).remove();});
				   }
				  alert(response_data.status+' : '+response_data.message);
			   }else{
				   //unknown error, response json improperly formatted
				  $("#" + response_data.holder_id + " > div.cancel img").attr("src",machform_path + "images/icons/delete.png");
				  alert(response_data.status+' : '+response_data.message);
			   }
